...
/**
 * An {@link Env} is a like a key-value property map where the keys are {@link Type}s and the values
 * instances of the key type.
 *
 * <p>To distinguish multiple values of the same {@link Type} a {@link Name} qualifier can be added
 * added.
 *
 * <p>To avoid collisions between qualified properties as used within different software modules
 * properties are usually defined local to the defining software module. During lookup the {@link
 * Class} of the processed module is used as the name-space key.
 *
 * <p>Properties that are considered namespace specific are resolved providing the {@link Class} of
 * the defining module or "bean".
 *
 * <p>Properties that are considered "globally" used do not use a particular namespace ({@code
 * null}).
 */
@FunctionalInterface
public interface Env {
  ...
  /**
   * Property name used to configure a boolean if {@link Verifier}s are tried to be resolved during
   * the binding process. If not, {@link Verifier#AOK} (no validation) is used (default).
   */
  String USE_VERIFICATION = "verify";

  /**
   * Boolean flag property which when set to {@code true} adds the bindings that were the basis of
   * an {@link Injector} or {@link Env} context as a {@link Resource} in the context that can be
   * resolved as array type of the binding class used.
   */
  String BIND_BINDINGS = "self-bind";

  <T> T property(Name qualifier, Type<T> property, Class<?> ns) throws InconsistentDeclaration;
  ...
  default <T> T property(String qualifier, Type<T> property, T defaultValue) {
    return Utils.orElse(defaultValue, () -> property(named(qualifier), property, null));
  }
  ...
  /**
   * @return An {@link Env} which isolates previous contributions using {@code with} from future
   *     ones. In other words the set of added properties added so far is not be changed by future
   *     {@code with} calls.
   */
  ...
  default <V> Env with(Name qualifier, Type<V> property, V value) {
    ...
    final class EnvWith implements Env {
      ...
      public <T> Env with(Name qualifier, Type<T> property, T value) {
        if (isolate) return new EnvWith(this, new HashMap<>()).with(qualifier, property, value);
      ...
      public String toString() {
        ...
        for (Map.Entry<Instance<?>, Object> e : values.entrySet()) {
          ...
          if (!e.getKey().name.isDefault()) str.append(" \"").append(e.getKey().name).append('"');
  ...
  default <E extends Enum<E>> Env withDependent(Class<E> set, E... elements) {
    Env res = this;
    for (E e : elements) {
      res = e == null ? res.with("null", set, null) : res.with(e.name(), raw(set), e);
    }
    return res;
  }
  ...
  default <T> Verifier verifierFor(T target) {
    if (!property(Env.USE_VERIFICATION, false)) return Verifier.AOK;
    @SuppressWarnings("unchecked")
    Class<T> targetClass = (Class<T>) target.getClass();
    Function<T, Verifier> factory =
        property(Cast.functionTypeOf(targetClass, Verifier.class), null);
