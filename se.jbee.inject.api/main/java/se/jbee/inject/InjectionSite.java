...
/**
 * Similar to a call-site each {@linkplain InjectionSite} represents the resolution of arguments
 * from a specific site or path that is represented by a {@link Dependency}. This is used in cases
 * where otherwise a dependency would be resolved over and over again for the same {@link
 * Dependency}. For example when injecting a factory {@link java.lang.reflect.Method} or {@link
 * java.lang.reflect.Constructor} with arguments.
 */
public final class InjectionSite {
  ...
  public InjectionSite(Injector context, Dependency<?> site, Hint<?>[] actualParameters) {
    this.site = site;
  ...
  public Object[] args(Injector context) throws UnresolvableDependency {
    if (lazyArgCount == 0) return preResolvedArgs;
    ...
    for (int j = 0; j < lazyArgCount; j++) {
      ...
      args[i] = generators[i] == null ? context.resolve(argDep) : generate(generators[i], argDep);
    }
    return args;
  }
  ...
  private int preResolveArgs(Injector context) {
    ...
    for (int i = 0; i < generators.length; i++) {
      ...
      if (hint.type().rawType == Injector.class) {
        ...
        Dependency<? extends Resource<?>> resourceDep =
            site //
                .typed(resourceTypeOf(ref.type))
                .named(ref.name)
                .at(hint.at);
        Resource<?> resource = context.resolve(resourceDep);
        if (resource.lifeCycle.isPermanent()) {
          // TODO and not has type variable involved
          preResolvedArgs[i] = generate(resource, site.onInstance(hint.relativeRef).at(hint.at));
        } else {
          lazyArgIndexes[lazyArgIndex++] = i;
