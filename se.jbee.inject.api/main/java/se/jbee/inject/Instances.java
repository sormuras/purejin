...
public final class Instances
    implements Qualifying<Instances>, Iterable<Instance<?>>, Serializable, Comparable<Instances> {
  ...
  public Instances push(Instance<?> top) {
    if (isAny()) return new Instances(top);
  ...
  public boolean moreQualifiedThan(Instances other) {
    ...
    for (int i = 0; i < hierarchy.length; i++) {
      if (hierarchy[i].moreQualifiedThan(other.hierarchy[i])) return true;
      if (other.hierarchy[i].moreQualifiedThan(hierarchy[i])) return false;
    }
    return false;
  }
