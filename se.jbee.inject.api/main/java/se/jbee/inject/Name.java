 ...
 * the same {@link Type}.
 */
public final class Name implements Qualifying<Name>, Serializable, Comparable<Name> {
  ...
  /**
   * Character used as divider for name-spacing {@link Class} objects passed to {@link
   * #named(Object)}.
   */
  public static final char NAMESPACE = ':';

  /**
   * Used when no name is specified. Maybe at first counter-intuitively this is the most qualified
   * name of all because it is the first to try. If the dependency asks for a specific name the
   * default name will not match an thus continue trying less qualifying names.
   *
   * @see #ANY
   */
  public static final Name DEFAULT = new Name("");
  /**
   * It is the least qualified name of all so it is the last name that will be tried and as it
   * matches any name asked for it the match is found.
   */
  ...
  public static <T> Name named(T name) {
    if (name == null) return named("null");
    @SuppressWarnings("unchecked")
    Function<T, String> naming = (Function<T, String>) ofType.get(name.getClass());
    return naming == null ? named(String.valueOf(name)) : named(naming.apply(name));
  }
  ...
  public boolean moreQualifiedThan(Name other) {
    ...
    if (thisIsDefault || otherIsDefault) return !otherIsDefault;
    // any is least qualified
    boolean thisIsAny = isAny();
    boolean otherIsAny = other.isAny();
    if (thisIsAny || otherIsAny) return !thisIsAny;
    boolean thisIsPattern = isPattern();
    boolean otherIsPattern = other.isPattern();
    if (thisIsPattern || otherIsPattern) return !thisIsPattern;
  ...
  private static boolean matches(String pattern, String str) {
    ...
    return j == strLen && (i == len || i + 1 == len && pattern.charAt(len - 1) == WILDCARD);
  }
}
