...
public final class Source implements Qualifying<Source>, Serializable, Comparable<Source> {
  ...
  private Source(
      Class<?> ident, DeclarationType declarationType, int declarationNo, int totalDeclarations) {
    this.ident = ident;
  ...
  public String toString() {
    return ident.getSimpleName() + "#" + declarationNo + ":" + declarationType.name().charAt(0);
  }
  ...
  public Source next() {
    totalDeclarations++;
    return declarationNo > 0 ? this : new Source(ident, declarationType, totalDeclarations, 0);
  }
}
