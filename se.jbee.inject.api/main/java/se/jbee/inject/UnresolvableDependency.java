...
/**
 * A {@link Dependency} that should be resolved during the injection or resolution process.
 *
 * <p>This is a grouping {@link RuntimeException} the problem specific exceptions are derived from.
 *
 * @see DependencyCycle
...
public abstract class UnresolvableDependency extends RuntimeException {
  ...
  public static final class DependencyCycle extends UnresolvableDependency {
    ...
    public DependencyCycle(Dependency<?> dependency, Locator<?> cycleTarget) {
   ...
   * the injected one. This is considered to be unintentional. Use a indirection like a provider or
   * services to resolve the problem.
   */
  public static final class UnstableDependency extends UnresolvableDependency {
  ...
  /**
   * An {@link Injector} couldn't find or identify the {@link Resource} that matches a {@link
   * Dependency} resolved. This is either because there is no matching {@link Resource} or because
   * it is unclear which {@link Resource} is requested by the {@link Dependency}.
   */
  public static final class ResourceResolutionFailed extends UnresolvableDependency {

    public <T> ResourceResolutionFailed(String msg, Dependency<T> dep, Resource<?>... available) {
      super(
          msg
              + "\n\t dependency: "
              + dep
    ...
    public ResourceResolutionFailed(Collection<Type<?>> types, List<?> dropped) {
      super(
  ...
  public static String describe(Resource<?>... rs) {
    ...
    for (Resource<?> rx : rs)
      b.append("\n\t- ").append(rx.signature.toString()).append(" from ").append(rx.source);
    return b.toString();
  }

  /**
   * A method has been described by its return and {@link Hint} {@link Type}s (e.g. for use as
   * factory or service) but such a method cannot be found. That usual means the defining class
  ...
  public static final class NoMethodForDependency extends UnresolvableDependency {
    ...
    public NoMethodForDependency(Type<?> returnType, Type<?>[] parameterTypes, Throwable cause) {
      super(
  ...
  /**
   * When trying to supply a {@link Dependency} (e.g. by calling a method or constructor) a error
   * occurred that prevented a successful supply.
   */
  ...
  public static final class IllegalAccess extends UnresolvableDependency {
    ...
    public <T> IllegalAccess(Locator<T> target, Dependency<? super T> dep) {
