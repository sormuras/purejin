...
/**
 * Describes WHAT (type-wise) can be generated by the backing {@link Resource} and WHERE
 * ("location") this should apply and be used.
 */
public final class Locator<T>
    implements Typed<T>, Qualifying<Locator<?>>, Serializable, Comparable<Locator<?>> {
  ...
  public boolean isUsableFor(Dependency<?> dep) {
    return isUsableNameWise(
            dep) // check names first since default goes sorts first but will not match any named
  ...
  /**
   * Does the {@link Type} of this {@link Locator} one that can generate an instance that is asked
   * for by the given {@link Dependency}?
   *
   * <p>OBS! This is not the classic type assignability. Partly the logic is reversed. A {@link
   * Locator} that declares {@code ? extends Number} expresses that be can create any and all kinds
   * of {@link Number}s. So a check would be reversed and check that the required type, say {@link
   * Integer} can be assigned to the offered type, here {@link Number}. Then {@link Integer} is an
   * available type.
   */
  ...
  private static boolean isGeneratableType(Type<?> required, Type<?> offered) {
    if (offered.isAssignableTo(required)) return true;
    if (offered.isUpperBound()) return required.isAssignableTo(offered);
    if (!required.rawType.isAssignableFrom(offered.rawType) || !offered.hasTypeParameter())
      return false;
    Type<?>[] offeredParams = offered.parameters();
    for (int i = 0; i < offeredParams.length; i++) {
      if (!isGeneratableTypeParameter(required.parameter(i), offeredParams[i])) return false;
    }
    return true;
  }

  private static boolean isGeneratableTypeParameter(Type<?> required, Type<?> offered) {
    if (offered.isUpperBound()) return required.isAssignableTo(offered);
    // (not a ? extends parameter)
    if (required.rawType != offered.rawType) return false;
  ...
  /**
   * Does this {@link Locator} provide the instance wanted by the given {@link Dependency}'s {@link
   * Name}
   */
  ...
  public Locator<T> indirect(boolean indirect) {
    return target.indirect == indirect ? this : new Locator<>(instance, target.indirect(indirect));
  }
  ...
  public boolean moreQualifiedThan(Locator<?> other) {
    return Qualifying.compareRelated(instance, other.instance, target, other.target);
  }
  ...
  /**
   * @since 8.1
   * @return a {@link Dependency} that {@link #isUsableFor(Dependency)} this {@link Locator}.
   */
  public Dependency<T> toDependency() {
    Dependency<T> dep = dependency(instance);
    if (target != Target.ANY) {
      if (!target.parents.isAny()) {
        for (Instance<?> p : target.parents) dep = dep.injectingInto(p);
      }
