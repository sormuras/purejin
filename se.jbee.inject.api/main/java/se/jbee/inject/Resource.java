...
/**
 * A {@link Resource} describes a injection situation or scenario through its {@link #signature} and
 * {@link #lifeCycle}. If the {@link Resource} applies to a actual {@link Dependency} situation its
 * {@link #generator} is used to create the instance injected should it not exist already.
 *
 * @since 8.1
 * @param <T> type of instances yielded by the {@link #generator}.
 */
public final class Resource<T>
    implements Comparable<Resource<?>>, Qualifying<Resource<?>>, Generator<T> {
  ...
  /**
   * The {@link Source} that that defined the binding which corresponds to this {@link Resource}).
   */
  public final Source source;

  /** The information on this {@link Scope} behaviour in relation to other {@link Scope}s. */
  public final ScopeLifeCycle lifeCycle;

  /**
   * The serial ID of this {@link Resource}. It is unique within the same {@link Injector} context
   * and assigned by the {@link Injector} implementation when it creates the cases during
   * initialisation.
   */
  public final int serialID;

  /**
   * Access to the annotations that should be considered in connection with this {@link Resource}.
   * For example the annotation of the {@link java.lang.reflect.Constructor} that internally is used
   * to create a instance.
   */
  public final Annotated annotations;

  /**
   * An optional verification check for this {@link Resource} executed at the end of the
   * bootstrapping phase.
   */
  ...
  public void init() {
    if (lifeCycle.isEager()) generate();
  }

  public Resource<T> withGenerator(Generator<T> generator) {
    return new Resource<>(serialID, source, lifeCycle, signature, annotations, verifier, generator);
  }
  ...
  public String toString() {
    return "#" + serialID + " " + signature + " " + source + " " + lifeCycle;
  }

  @Override
  public boolean equals(Object obj) {
    return obj instanceof Resource && ((Resource<?>) obj).serialID == serialID;
  }
  ...
  public int compareTo(Resource<?> other) {
    ...
    if (aRaw != bRaw) {
      if (aRaw.isAssignableFrom(bRaw)) return 1;
      if (bRaw.isAssignableFrom(aRaw)) return -1;
      return aRaw.getName().compareTo(bRaw.getName());
    }
    // secondly any implicit bind is always after any other type of bind
    if (source.declarationType == IMPLICIT && other.source.declarationType != IMPLICIT) return 1;
    if (source.declarationType != IMPLICIT && other.source.declarationType == IMPLICIT) return -1;
