...
public final class Dependency<T> implements Typed<T>, Iterable<Injection>, Serializable {

  /**
   * A empty {@link Injection} hierarchy. It is used whenever the {@link Dependency} does not depend
   * on the actual hierarchy. This is the default.
   */
  ...
  public String toString() {
    ...
    b.append(instance);
    for (int i = hierarchy.length - 1; i >= 0; i--) b.append(" <= ").append(hierarchy[i].target);
    return b.toString();
  }
  ...
  public Dependency<T> ignoredScoping() {
    return hierarchy.length == 0
        ? this
        : dependency(instance, arrayMap(hierarchy, Injection::ignoredScoping));
  }
  ...
  public Instance<?> target(int level) {
    return level >= hierarchy.length ? Instance.ANY : injection(level).target.instance;
  }
   ...
   * @return a new {@link Dependency} similar to this with the given target {@link Class} added to
   *     the top of its {@link #hierarchy}
   */
  ...
  public Dependency<T> injectingInto(Locator<?> target, ScopeLifeCycle lifeCycle)
      throws DependencyCycle, UnstableDependency {
    Injection injection = new Injection(instance, target, lifeCycle);
    if (hierarchy.length == 0) return new Dependency<>(at, instance, injection);
  ...
  private void ensureStableScopeNesting(Injection injection) throws UnstableDependency {
    Injection unstable =
        arrayFindFirst(hierarchy, e -> !injection.lifeCycle.isConsistentIn(e.lifeCycle));
    if (unstable != null) throw new UnstableDependency(unstable, injection);
  }

  public void ensureNoIllegalDirectAccessOf(Locator<? extends T> locator) {
    if (!locator.target.indirect) return;
    Type<? super T> required = instance.type;
    if (required.rawType.isInterface()) return;
    for (int level = 0; level < injectionDepth(); level++) {
      ...
      if (parent.type.rawType.isInterface()) return;
    }
    throw new UnresolvableDependency.IllegalAccess(locator, this);
  }
