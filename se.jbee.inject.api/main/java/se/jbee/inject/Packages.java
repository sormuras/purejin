...
public final class Packages implements Qualifying<Packages>, Serializable, Comparable<Packages> {
  ...
  private static String[] packageNamesOf(
      Class<?> packageOf, String suffix, Class<?>... packagesOf) {
  ...
  private static String packageNameOf(Type<?> packageOf) {
    return packageOf.isUpperBound() ? "-NONE-" : packageNameOf(packageOf.rawType);
  }
  ...
  public Packages and(Packages further) {
    ...
    return new Packages(
        set.toArray(new String[0]), includingSubpackages || further.includingSubpackages);
  }

  public Packages parents() {
    if (rootDepth == 0) return this;
    if (rootDepth == 1) return includingSubpackages ? ALL : DEFAULT;
    return new Packages(arrayMap(roots, Packages::parent), includingSubpackages);
  }
  ...
  private static String parent(String root) {
    return root.substring(
        0, root.lastIndexOf('.', root.length() - 2) + (root.endsWith(".") ? 1 : 0));
  }
  ...
  private static void commonPackageDepth(Class<?> type, Class<?>[] types) {
    if (arrayContains(
        types,
        type,
        (a, b) -> dotsIn(a.getPackage().getName()) != dotsIn(b.getPackage().getName())))
    ...
    return arrayContains(
        ...
        root ->
            seqRegionEquals(
                root,
                packageNameOfType,
                includingSubpackages ? root.length() : packageNameOfType.length()));
  }
  ...
  public boolean moreQualifiedThan(Packages other) {
    if (includingSubpackages != other.includingSubpackages) return !includingSubpackages;
    return rootDepth != other.rootDepth && rootDepth > other.rootDepth;
  }

  @Override
  public int compareTo(Packages other) {
    int res = Boolean.compare(includingSubpackages, other.includingSubpackages);
    if (res != 0) return res;
  ...
  public String toString() {
    if (roots.length == 0) return includingSubpackages ? "*" : "(default)";
    if (roots.length == 1) return toString(roots[0]);
    StringBuilder b = new StringBuilder();
    for (String root : roots) b.append('+').append(toString(root));
