 ...
 *
 * <p>This can be restricted by the {@link Packages} the injected that is injected is defined in or
 * the {@link Type} of the receiving instance.
 */
@SuppressWarnings("squid:S1448")
public final class Target implements Qualifying<Target>, Serializable, Comparable<Target> {
  ...
  /**
   * When true, access to this {@link Target} is only permitted in when it is used though an
   * interface.
   */
  public final boolean indirect;

  private Target(Instances parents, Instance<?> instance, Packages packages, boolean indirect) {
  ...
  public Target indirect(boolean indirect) {
    return this.indirect == indirect ? this : new Target(parents, instance, packages, indirect);
  }
  ...
  /**
   * @return true if this {@link Target} matches any and all {@link Dependency}s, in other words it
  ...
  /**
   * @return true in case the actual types of the injection hierarchy are assignable with the ones
   *     demanded by this target.
   */
  public boolean isUsableInstanceWise(Dependency<?> dep) {
    return (instance.isAny() || isUsableInstanceWise(dep.target())) && isUsableParentWise(dep);
  }
  ...
  private static boolean isSuitableInstance(Instance<?> required, Instance<?> offered) {
  ...
  public boolean moreQualifiedThan(Target other) {
    ...
    if (l > 0) { // length is known to be equal
      if (parents.moreQualifiedThan(other.parents)) return true;
      if (other.parents.moreQualifiedThan(parents)) return false;
    }
    return Qualifying.compareRelated(instance, other.instance, packages, other.packages);
  }
  ...
  public boolean equalTo(Target other) {
    return this == other
        || packages.equalTo(other.packages)
