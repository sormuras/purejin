...
/**
 * A {@link Hint} is a suggested reference for parameters of a {@link Constructor} or {@link Method}
 * to inject.
 *
 * @since 8.1
...
public final class Hint<T> implements Typed<T>, Descriptor {
  ...
  public static Hint<?>[] signature(Class<?>... parameterTypes) {
    if (parameterTypes.length == 0) return none();
    return Arrays.stream(parameterTypes).map(Hint::relativeReferenceTo).toArray(Hint[]::new);
  }
  ...
  /**
   * A {@link Type} reference is relative to the injection site hierarchy. It can be pre-resolved
   * for each site.
   *
  ...
  /**
   * An {@link Instance} reference is relative to the injection site hierarchy. It can be
   * pre-resolved for each site.
   *
   * @param target The {@link Instance} to resolve as parameter within the hierarchy
   * @return A {@link Hint} representing the relative reference
   */
  ...
  /**
   * A {@link Dependency} reference is absolute. That means it ignores the injection site hierarchy.
   *
  ...
  public static <T> Hint<T> constant(T constant) {
    if (constant == null) throw InconsistentDeclaration.incomprehensibleHint(null);
  ...
  public static int indexForType(Type<?>[] types, Hint<?> hint, Hint<?>[] args) {
    Type<?> target = hint.type();
    // 1. lowest index with exact type match, or else
    for (int i = 0; i < types.length; i++)
      if (args[i] == null && target.equalTo(types[i])) return i;
    // 2. lowest index with same raw type and assignable, or else
    for (int i = 0; i < types.length; i++)
      if (args[i] == null
          && target.rawType == types[i].rawType //
          && target.isAssignableTo(types[i])) return i;
    // 3. lowest index with assignable type, or else
    for (int i = 0; i < types.length; i++)
      if (args[i] == null && target.isAssignableTo(types[i])) return i;
    // 4. not found
    return -1;
  }
  ...
  private Hint(
      Type<T> asType,
      T value,
      Instance<? extends T> relativeRef,
  ...
  /**
   * @return a more qualified {@link Hint} if the provided type is more qualified which changes the
   *     references but does not change {@link #asType} in case that was intentionally set to a
   *     supertype.
   *     <p>At the point this method is called the {@link #asType} already had its effect but we
   *     still want to preserve the information for debugging so we do not get confused.
   */
  ...
  public <E> Hint<E> typed(Type<E> type) {
    Instance<? extends E> newRelRef = relativeRef == null ? null : relativeRef.typed(type);
    Dependency<? extends E> newAbsRef = absoluteRef == null ? null : absoluteRef.typed(type);
    return new Hint<>(type, (E) value, newRelRef, newAbsRef, at);
  }

  @Override
  public String toString() {
    if (isConstant()) return "value as " + asType;
    return "ref to " + (absoluteRef != null ? absoluteRef : relativeRef) + " as " + asType;
  }

  @Override
  public boolean equals(Object obj) {
    if (this == obj) return true;
    if (!(obj instanceof Hint)) return false;
    Hint<?> other = (Hint<?>) obj;
    if (isConstant()) return value == other.value;
    if (!asType.equalTo(other.asType)) return false;
    if (relativeRef != null)
  ...
  public T resolveIn(Injector context) {
    if (asType.rawType == Injector.class) return (T) context;
    if (isConstant()) return value;
    if (absoluteRef != null) return context.resolve(absoluteRef);
    // relative ref
