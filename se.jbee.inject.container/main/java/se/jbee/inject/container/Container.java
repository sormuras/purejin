...
/**
 * The default {@link Injector} implementation that is based on {@link Resources} created from
...
public final class Container implements Injector, Env {
  ...
  private Container(ResourceDescriptor<?>... descriptors) {
    this.resources =
        new Resources(this::supplyInContext, scope -> resolve(scope, Scope.class), descriptors);
    this.liftResources =
        new LiftResources(
            orElse((t, arr) -> arr, () -> resolve(Lift.Sequencer.class)),
  ...
  private Observer resolvePostConstructObserver() {
    return Observer.merge(resolve(Observer[].class));
  }
  ...
  public <T> T property(Name qualifier, Type<T> property, Class<?> ns) {
    try {
      ...
      if (ns == null) return resolve(global);
  ...
  public <T> T resolve(Dependency<T> dep) {
    ...
    if (rawType == Injector.class && (dep.instance.name.isAny() || dep.instance.name.isDefault()))
      return (T) builtUp;
    if (rawType == Env.class && dep.instance.name.equalTo(Name.AS)) return (T) this;
    return resolveFromResource(dep, type, rawType);
  }

  @SuppressWarnings("unchecked")
  private <T> T resolveFromResource(Dependency<T> dep, final Type<T> type, final Class<T> rawType) {
    boolean isResourceResolution = rawType == Resource.class || rawType == Generator.class;
    if (isResourceResolution) {
      ...
      if (match != null) return match.generate(dep);
    }
    if (type.arrayDimensions() == 1) return resolveArray(dep, type.baseType());
    if (isResourceResolution) return (T) resolveFromUpperBound(dep.onTypeParameter());
    return (T)
        resolveFromUpperBound(dep) //
            .generate((Dependency<Object>) dep);
  }

  /**
   * There is no direct match for the required type but there might be a wild-card binding, that is
   * a binding capable of producing all sub-types of a certain super-type.
   */
  private <T> Resource<?> resolveFromUpperBound(Dependency<T> dep) {
    Type<T> type = dep.type();
    Resource<?> match =
        arrayFindFirst(
            resources.forType(Type.WILDCARD),
            r ->
                type.isAssignableTo(r.type())
  ...
  private static <T> Resource<T> mostQualifiedMatchIn(Resource<T>[] rs, Dependency<T> dep) {
  ...
  private <T> ResourceResolutionFailed noResourceFor(Dependency<T> dep) {
    Type<T> type = dep.type();
    Type<?> listType =
        type.rawType == Resource.class || type.rawType == Generator.class
            ? type.parameter(0)
            : type;
    return new ResourceResolutionFailed(
        "No matching resource found.", dep, resources.forType(listType));
  }

  private static <T> void throwAmbiguousDependency(Dependency<T> dep) {
    throw new ResourceResolutionFailed(
        "Resolving any instance for any type is considered too ambiguous to be intentional", dep);
  }
  ...
  private <T, E> T resolveArray(Dependency<T> dep, Type<E> elemType) {
    ...
    if (dep.type().rawType.getComponentType().isPrimitive())
      throw new ResourceResolutionFailed(
          "Primitive arrays cannot be used to inject all instances of the wrapper type. Use the"
              + " wrapper array instead.",
          dep);
    Set<Integer> identities = new HashSet<>();
    if (!elemType.isUpperBound()) {
      List<E> elements = new ArrayList<>();
      Resource<E>[] elemResources = resources.forType(elemType);
      if (elemResources != null) addAllMatching(elements, identities, dep, elemType, elemResources);
    ...
    for (Entry<Class<?>, Resource<?>[]> e : resources.entrySet())
      if (Type.raw(e.getKey()).isAssignableTo(elemType))
        addAllMatching(elements, identities, dep, elemType, (Resource<? extends E>[]) e.getValue());
    return toArray(elements, elemType);
  }

  @SuppressWarnings("unchecked")
  private <R, T> Resource<T>[] resolveArrayElementResources(
      Dependency<R> resourceDep, Type<T> generatedType) {
    ...
    return candidates == null
        ? new Resource[0]
        : arrayFilter(candidates, c -> c.signature.isUsableInstanceWise(dep));
  }
  ...
  private <T> Resource<T>[] resolveArrayElementResourcesForUpperBoundType(
    ...
    for (Entry<Class<?>, Resource<?>[]> e : resources.entrySet())
      if (raw(e.getKey()).isAssignableTo(generatedType))
        addCompatibleResources(res, dep, (Resource<? extends T>[]) e.getValue());
    return toArray(res, raw(Resource.class));
  }

  private static <T> void addCompatibleResources(
      List<Resource<?>> res, Dependency<T> dep, Resource<? extends T>[] candidates) {
    for (Resource<? extends T> candidate : candidates)
      if (candidate.signature.isUsableInstanceWise(dep)) res.add(candidate);
  }

  private static <E, T> void addAllMatching(
      List<E> elements,
    ...
    for (Resource<? extends E> elemResource : elementResources) {
      if (elemResource.signature.isUsableFor(elemDep)) {
        ...
        if (identities.add(identityHashCode(instance))) elements.add(instance);
      }
    }
  }
  ...
  /**
   * Can be called by a {@link Generator} to create an instance from a {@link Supplier} and have
   * {@link Lift}s applied for it as well as notifying {@link Observer}s.
   */
  private <T> T supplyInContext(
      Dependency<? super T> injected, Supplier<? extends T> supplier, Resource<T> resource) {
    Injector context = getBuiltUp();
    T instance = supplier.supply(injected, context);
    if (instance != null && !resource.lifeCycle.scope.equalTo(Scope.reference)) {
      if (liftResources != null) instance = liftResources.lift(instance, injected, context);
      if (observer != null && resource.lifeCycle.isPermanent()) {
