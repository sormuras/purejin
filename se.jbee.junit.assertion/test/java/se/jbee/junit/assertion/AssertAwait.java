...
/**
 * This is a small and simple utility to assert things about asynchronously changing conditions.
 *
 * <p>It is inspired by the {@code awaitility} library but works on top of {@code junit5} and only
 * has very essential functionality.
 */
final class AssertAwait {
  ...
  public static void assertAwait(Await await, Callable<AssertionFailedError> test) {
    ...
    while (waitTimeLeftMillis > 0) {
      try {
        ...
        if (executor.isShutdown() || executor.isTerminated()) fail("await execution was canceled.");
        try {
          lastError = error.get(waitTimeLeftMillis, TimeUnit.MILLISECONDS);
          long timePassed = currentTimeMillis() - waitStarted;
          if (lastError == null) {
            if (timePassed < minWaitMillis)
              fail(
                  format(
                      "await condition met before minimum time %s had passed", await.minWaitTime));
            return; // success
          }
        ...
        if (error != null) error.cancel(true);
      }
    }
    if (lastError != null) throw lastError;
    failNotCompleted(await);
  ...
  private static void failNotCompleted(Await await) {
    fail(
        format("await never completed a test execution in the maximum time %s", await.maxWaitTime));
  }
