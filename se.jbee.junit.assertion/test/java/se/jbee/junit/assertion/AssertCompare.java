...
final class AssertCompare {
  ...
  static <T extends Comparable<T>> void assertGreaterThanOrEqual(
      T expected, T actual, Supplier<String> message) {
    if (actual.compareTo(expected) < 0) failCompare(actual, ">=", expected, message);
  }
  ...
  static <T extends Comparable<T>> void assertLessThanOrEqual(
      T expected, T actual, Supplier<String> message) {
    if (actual.compareTo(expected) > 0) failCompare(actual, "<=", expected, message);
  }
  ...
  static <T extends Comparable<T>> void assertGreaterThan(
      T expected, T actual, Supplier<String> message) {
    if (actual.compareTo(expected) <= 0) failCompare(actual, ">", expected, message);
  }
  ...
  static <T extends Comparable<T>> void assertLessThan(
      T expected, T actual, Supplier<String> message) {
    if (actual.compareTo(expected) >= 0) failCompare(actual, "<", expected, message);
  }
