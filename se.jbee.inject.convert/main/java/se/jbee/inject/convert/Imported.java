...
/**
 * A {@link Imported} is the dynamic equivalent of a Java source file's <code>import</code> context
 * which allows to use simple names instead of fully qualified class names.
 *
 * <p>The utility allows to resolve {@link Type}s from {@link String}s with simple type names using
 * {@link #resolve(String)}. To expand the simple name a import context is set up in the {@link
 * Imported} using the {@link #add(Class...)} and {@link #add(Imports)} methods.
 *
 * <p>The {@link #base()} {@link Imported} has most common JRE {@link Class}es added. To not
 * override a {@link Imported} context the {@link #fork()} method is used to create a copy which
 * then can be modified using {@link #add(Class...)}.
 *
 * <p>This class is not thread-safe as it is not intended to be used in a multi-threaded fashion.
 * Instead threads should use the {@link #fork()} method should they start from a shared {@link
 * Imported} base.
 *
 * <h4>Example Inputs</h4>
 *
 * <p>All of the following input are valid and converted to the expected {@link Type} value.
 *
 * <pre>
 * String
 * List&lt;String&gt;
...
public final class Imported {
  ...
  private static final Imported BASE =
      empty()
          .add(
              String.class,
              CharSequence.class,
  ...
  /**
   * @return A {@link #fork()} of the standard {@link Imported} context containing common JRE type
   *     'imports'.
   */
  ...
  public Imported add(Class<?>... types) {
    for (Class<?> imported : types)
      baseTypeBySimpleName.put(imported.getSimpleName(), Type.classType(imported));
    return this;
  }

  /**
   * @return A new, forked instance of this {@link Imported} context that will not be affected by
   *     further changes to its origin context.
   */
  ...
  private Type<?> resolveUnknownType(String signature) {
    ...
    for (int i = 0; i < arrayDimensions; i++) res = res.addArrayDimension();
    return res;
  }
      ...
      if (end < 0)
        throw new IllegalArgumentException(
            "Expected a "
                ...
                + rawTypeName
                + " but there seems to be non in "
                + signature);
      actualTypeParameters[i] = resolve(signature.substring(start + 1, end));
      start = end;
      if (i == expectedTypeParameters - 1 && signature.charAt(end) != '>') {
        illegalAt("Expected end of generic type arguments", signature, end);
      }
    }
  ...
  private Type<?> resolveNonGenericNonArrayType(String signature) {
    ...
    if (type == null)
      throw new IllegalArgumentException("Missing import for type: `" + signature + "`");
    return type;
  }
