...
/**
 * A {@link ConvertTo} groups {@link Converter}s that all convert to the same output type from
 * different input types.
 *
 * <p>It makes use of {@link Converter} chains to allow for as many input types based on the know
 * type conversions.
 *
...
public final class ConvertTo<B> {
  ...
  public ConvertTo(Converter<?, B> tail, Injector context) {
    ...
    Type<?> converterType = raw(tailType).toSuperType(Converter.class);
  ...
  private void initAnnotatedChains(Class<?> tailType, Type<?> directInputType, Injector context) {
    Imported imports = Imported.base().add(tailType.getAnnotation(Imports.class));
    if (tailType.isAnnotationPresent(Converts.class)) {
      for (Converts converts : tailType.getAnnotationsByType(Converts.class)) {
        Imported localImports = imports;
        if (converts.imports().length > 0) localImports = imports.fork().add(converts.imports());
        Instance<?>[] chain = instanceChain(localImports, converts.value());
        Deque<Link<?, ?>> converterChain = converterChain(chain, directInputType, context);
        Link<?, ?> tail = converterChain.pollLast();
        Converter<?, B> chainBuilder = ins.get(directInputType);
        while (tail != null) {
  ...
  static Deque<Link<?, ?>> converterChain(
      Instance<?>[] chain, Type<?> directInputType, Injector context) {
  ...
  private static <A, B> Link<A, B> createLink(Type<A> in, Instance<B> out, Injector context) {
    return new Link<>(
        in, out.type, context.resolve(out.name, Converter.converterTypeOf(in, out.type)));
  }
  ...
  static Instance<?>[] instanceChain(Imported imports, String[] chain) {
    ...
    for (int i = 0; i < chain.length; i++) {
      if (chain[i].indexOf(' ') >= 0) {
        ...
        chainInstances[i] =
            Instance.instance(Name.named(nameAndType[0]), imports.resolve(nameAndType[1]));
      } else {
  ...
  public <A> Converter<A, B> from(Type<A> in) {
    ...
    if (res == null)
      throw new UnsupportedOperationException("Conversion from " + in + "not available");
    return res;
  }
