 ...
 *
 * <p>Any builder is just a utility to construct calls to {@link #add(Env, Binding)}
 *
...
public final class Bindings {
  ...
  public <T> void add(Env env, Binding<T> complete) {
    if (!complete.isComplete()) throw InconsistentBinding.addingIncomplete(complete);
  ...
  public <V extends Descriptor> void addExpanded(Env env, Binding<?> binding, V value) {
    ...
    @SuppressWarnings("unchecked")
    ValueBinder<V> binder = env.property(raw(ValueBinder.class).parameterized(classType(type)));
    if (binder == null) throw InconsistentBinding.undefinedValueBinderType(binding, type);
  ...
  public void addAnnotated(Env env, Class<?> annotated) {
    ...
    // should be doable by just defining a module that tries to resolve the annotations from the
    // environment => confirm with a test
    for (Annotation a : as) if (addsAnnotatedType(env, annotated, a)) n++;
    for (Method m : annotated.getMethods())
      for (Annotation a : m.getDeclaredAnnotations()) if (addsAnnotatedMethod(env, m, a)) n++;
    if (n == 0) throw InconsistentBinding.noAnnotationModule(annotated);
  }

  private boolean addsAnnotatedType(Env env, Class<?> annotated, Annotation annotation) {
    ModuleWith<Class<?>> then =
        env.in(annotated)
            .property(
                named(annotation.annotationType()).toString(), ModuleWith.TYPE_ANNOTATION, null);
  ...
  private boolean addsAnnotatedMethod(Env env, Method annotated, Annotation annotation) {
    ModuleWith<Method> then =
        env.in(annotated.getDeclaringClass())
            .property(
                named(annotation.annotationType()).toString(), ModuleWith.METHOD_ANNOTATION, null);
    if (then == null) return false;
    then.declare(this, env, annotated);
    return true;
  }
  ...
  public void declareFrom(Env env, Module... modules) {
    ...
    for (Module m : modules) {
      ...
      if (hasBeenDeclared && !isClassConceptStateless(ns)) stateful.add(ns);
  ...
  /**
   * In contrast to {@link #supplyConstant(Object)} which does supply the constant as {@link
   * Generator} and thereby does not support custom {@link ScopeLifeCycle} or {@link Scope}s this
   * way of supplying the constant will treat the constant as bean, that is like any "dynamically"
   * supplied value.
   *
   * @param <T> type of the constant
   * @param constant a bean that requires {@link ScopeLifeCycle} effects.
   * @return A {@link Supplier} that supplies the given constant with {@link ScopeLifeCycle}
   *     effects.
   */
  ...
  /**
   * Since the {@link Supplier} also implements {@link Generator} it is used directly without any
   * {@link ScopeLifeCycle} effects. Effectively a constant always has the {@link Scope#container}.
   *
   * <p>The implementation also implements {@link #equals(Object)} and {@link #hashCode()} to allow
   * elimination of duplicate constant bindings.
   */
  private static final class ConstantSupplier<T> implements Supplier<T>, Generator<T> {
    ...
    public boolean equals(Object obj) {
      return obj instanceof ConstantSupplier && constant == ((ConstantSupplier<?>) obj).constant;
    }
