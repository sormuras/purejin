...
/**
 * A {@link ModuleWith} is an extension to a usual {@link Module} that depends on *one* of the
 * values that have been set in the {@link Env}. The property is resolved by type.
 *
 * <p>{@link ModuleWith} are also used to apply the effects of custom {@link Annotation}s. In that
 * case the property passed is the annotated {@link Class}.
 *
 * @see Module
...
public interface ModuleWith<T> extends Module {
   ...
   * @param property The set value for the type (maybe null in case no value was set or the value
   *     was set to null)
   */
  ...
  default void declare(Bindings bindings, Env env) {
    ...
    final T value =
        valueType.rawType == Env.class
            ? (T) env
            : (T) env.property(Name.DEFAULT, valueType, getClass());
