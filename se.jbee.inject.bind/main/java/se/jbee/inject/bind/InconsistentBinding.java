...
public final class InconsistentBinding extends InconsistentDeclaration {
  ...
  public static InconsistentBinding addingIncomplete(Binding<?> complete) {
    return new InconsistentBinding("Attempt to add an incomplete binding: " + complete);
  }

  public static InconsistentBinding undefinedValueBinderType(Binding<?> expanded, Class<?> macro) {
    return new InconsistentBinding(
        "Attempt to expand value of type "
            + macro.getName()
            + " that is not bound to a macro for binding: "
            + expanded);
  }

  public static InconsistentBinding undefinedEnvProperty(Name name, Type<?> property, Class<?> ns) {
    return new InconsistentBinding(
        "Missing environment property: " + name + " of type " + property + " in " + ns);
  }

  public static InconsistentBinding illegalCompletion(Binding<?> completing, BindingType type) {
    return new InconsistentBinding(
        "Attempt to complete a binding with illegal type " + type + " :" + completing);
  }
  ...
  public static InconsistentBinding referenceLoop(
      Binding<?> inconsistent, Instance<?> linked, Instance<?> bound) {
    return new InconsistentBinding(
  ...
  public static InconsistentBinding noAnnotationModule(Class<?> type) {
    return new InconsistentBinding(
        "Expected at least one annotation used on type that is bound to an annotation module"
            + " declaration: "
            + type.getName());
  }
  ...
  public static void nonnullThrowsReentranceException(Object field) {
    if (field != null) throw contextAlreadyInitialised();
  }
}
