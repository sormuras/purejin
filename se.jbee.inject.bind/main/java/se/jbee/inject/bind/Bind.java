...
public final class Bind {
  ...
  public static final Bind UNINITIALIZED = new Bind(null, null, null, Scope.mirror, Target.ANY);
  ...
  private Bind(Env env, Bindings bindings, Source source, Name scope, Target target) {
  ...
  public Bind next() {
    return source == null ? this : new Bind(env, bindings, source.next(), scope, target);
  }

  public <T> Binding<T> asType(
      Locator<T> locator, BindingType type, Supplier<? extends T> supplier) {
    return Binding.binding(locator, type, supplier, effectiveScope(locator), source);
  }

  private <T> Name effectiveScope(Locator<T> locator) {
    Name effectiveScope =
        scope.equalTo(Scope.mirror) //
            ? env.property(ScopesBy.class).reflect(locator.type().rawType)
            : scope;
    return effectiveScope.equalTo(Scope.auto) ? Scope.application : effectiveScope;
  }
}
