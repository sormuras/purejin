...
/**
 * A {@link Binding} is a {@link ResourceDescriptor} created during the bootstrapping process based
 * on {@link Bindings}, {@link Bundle}s and {@link Module}s.
 *
...
public final class Binding<T> extends ResourceDescriptor<T>
  ...
  public static <T> Binding<T> binding(
      ...
      Source source) {
    return new Binding<>(
        signature, type, supplier, scope, source, annotatedOf(supplier), Verifier.AOK);
  }
  ...
  public Binding<T> annotatedBy(Annotated annotations) {
    if (annotations == this.annotations) return this; // just a optimisation for a likely case
    return new Binding<>(signature, type, supplier, scope, source, annotations, verifier);
  }

  @Override
  public Binding<T> verifiedBy(Verifier verifier) {
    if (verifier == this.verifier) return this;
    return new Binding<>(signature, type, supplier, scope, source, annotations, verifier);
  }

  public Binding<T> complete(BindingType type, Supplier<? extends T> supplier) {
    if (type == BindingType.VALUE) throw InconsistentBinding.illegalCompletion(this, type);
    Name effectiveScope = type == BindingType.REFERENCE ? Scope.reference : scope;
    return new Binding<>(
        signature, type, supplier, effectiveScope, source, annotatedOf(supplier), verifier);
  }
  ...
  public int compareTo(Binding<?> other) {
    int res =
        signature.type().rawType.getName().compareTo(other.signature.type().rawType.getName());
    if (res != 0) return res;
