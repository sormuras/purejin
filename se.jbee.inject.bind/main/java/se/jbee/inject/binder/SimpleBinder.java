...
/**
 * A simple fluent binder API that only allows to bind to constants or provided {@link Supplier}s,
 * {@link Generator}s or factory methods.
 *
 * <p>The main role of the simple API is to be used during the bootstrapping of the default {@link
 * Env} where no property from the {@link Env} is available. Therefore this API does not use any of
 * the abstractions between the fluent API and the {@link Bindings} (like {@link ValueBinder}s)
 * since these would require an already defined {@link Env}.
 *
 * @since 8.1
 */
public class SimpleBinder
    implements InstanceLocalBinder<SimpleBinder>,
  ...
  public static final class TypedSimpleBinder<T> implements SupplierBinder<T>, ReferenceBinder<T> {
    ...
    private void addBindingWith(Supplier<? extends T> supplier) {
      bind.bindings.add(
          bind.env, bind.asType(new Locator<>(bound, bind.target), PREDEFINED, supplier));
    }
  }
}
