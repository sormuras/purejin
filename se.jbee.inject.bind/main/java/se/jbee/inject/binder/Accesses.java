...
/**
 * Accesses a value that is extracted from a {@link Field}. The {@link Field} is read each time the
 * {@link Supplier} supplying is asked to supply the value. In other words it depends on {@link
...
public final class Accesses<T> extends ReflectiveDescriptor<Field, T> implements Supplier<T> {
  ...
  private static Type<?> actualType(Object owner, Field target) {
    return requiresActualReturnType(target, Field::getType, Field::getAnnotatedType) //
   ...
   *
   * <p>So an {@link Accesses} can be used as the {@link Supplier} but it can also just be used as
   * the {@link Descriptor} and data to create another {@link Supplier}.
   */
  @SuppressWarnings("unchecked")
  @Override
  public T supply(Dependency<? super T> dep, Injector context) {
    Object instance = isHinted() ? getAsHint().resolveIn(context) : as;
    Get get = context.resolve(dependency(Get.class).injectingInto(target.getDeclaringClass()));
    try {
