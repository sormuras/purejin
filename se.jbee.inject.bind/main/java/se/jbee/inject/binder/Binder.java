...
/**
 * The default implementation of a fluent binder API that provides a lot of utility methods to
...
public class Binder {
  ...
  /**
   * Adds bindings indirectly by inspecting the annotations present on the provided types and their
   * methods. An annotation can be linked to a binding pattern described by a {@link ModuleWith}
   * linked to a specific annotation as part of the {@link Env}. This can done either
   * programmatically or via {@link java.util.ServiceLoader} entry for the {@link ModuleWith} (of
   * {@link Class}).
   *
   * <p>This is the most "automatic" way of binding that maybe has closes similarity with annotation
   * based dependency injection as found in CDI or spring. It separates the specifics of the bind
   * from the target. While this is very simple to use it is also very limiting.
   *
   * @param types all types to bind using annotations present on the type and their methods
   */
  ...
  /**
   * All bindings made with the returned {@link Binder} will only allow access (injection/resolving)
   * when it is done via an interface type.
   *
  ...
  /**
   * Binds all implemented types that are considered an API by the {@link PublishesBy} strategy
   * defined in the {@link Env}.
   *
   * <p>For example binding {@link Integer} with {@link PublishesBy#SUPER} (all super classes) adds
   * references that bind {@link Number} to {@link Integer}, {@link java.io.Serializable} to {@link
   * Integer} and so forth for all types it does implement.
   *
   * <p>Should these reference clash with another explicit binding, for example {@link Number} was
   * bound to some other value provider, the explicit binding takes precedence. Also several
   * contract-bound bindings from the same type to different implementors do not clash and are
   * removed because they are ambiguous. So usually using {@code withContractAccess} does not create
   * issues with clashing bindings.
   *
   * <p>Note also that the usage of this "modifier" allows to declare combinations that do not make
   * much sense. For example using a {@code to}-clause that refers to a sub-type.
   *
   * @return a {@link Binder} that binds all subsequent {@code bind} calls not as the exact type but
   *     as all published APIs types.
   */
  ...
  /**
   * Binds all implemented types that are considered an API by the provided {@link PublishesBy}
   * strategy.
   *
   * <p>Note that this means APIs added to the environment are not considered. Only the provided
   * {@link PublishesBy} strategy is considered.
   *
   * @param strategy The predicate to find out what supertypes to consider APIs of a given
   *     implementation type and which around bound to the implementation as a consequence of it.
   * @return a {@link Binder} that binds all subsequent {@code bind} calls not as the exact type but
   *     as all published APIs types.
   */
   ...
   *
   * <p>Note that this method is only used in case an array type should be bound explicitly. To make
   * several independent bindings that can be injected as array, set or list use {@link
   * #multibind(Name, Type)}s.
   *
  ...
  /**
   * Same as {@link #bind(Name, Type)} just that both arguments are provided in form of an {@link
   * Instance}.
   *
   * @see #bind(Name, Type)
   ...
   *
   * <p>Just a short from of {@code bind(type).toConstructor()}.
   *
   * @param type both the implementation type and the type the created instance(s) should be known
   *     as
   */
   ...
   *
   * <p>Just a short from of {@code bind(instance).toConstructor()}.
   *
   * @param instance both the implementation type and the name and type the created instance(s)
   *     should be known as
   */
   ...
   * Construct a named instance of the provided type.
   *
   * <p>Just a short from of {@code bind(name, type).toConstructor()}.
   *
  ...
  public final ConnectBinder connect() {
    Env env = env();
    return connect(
        env.property(CONNECT_QUALIFIER, ProducesBy.class, env.property(ProducesBy.class)));
  }
  ...
  public <T> ConnectTargetBinder<T> connect(Class<T> api) {
    return new ConnectTargetBinder<>(this, ProducesBy.OPTIMISTIC.in(api), raw(api));
  }
   ...
   */
  @SafeVarargs
  public final void installIn(Class<?> subContext, Class<? extends Bundle>... lazyInstalled) {
    installIn(subContext.getName(), lazyInstalled);
  }

  /**
   * Binds a lazy {@link Bundle} which is installed in a sub-context {@link Injector}. The {@link
   * Injector} is bootstrapped lazily on first usage. Use {@link Injector#subContext(String)} to
   * resolve the sub-context by name.
   *
   * @param subContext the name of the lazy {@link Injector} sub-context
   * @param lazyInstalled the {@link Bundle} to install in the sub-context lazy {@link Injector}
   * @since 8.1
   */
  @SafeVarargs
  public final void installIn(String subContext, Class<? extends Bundle>... lazyInstalled) {
  ...
  /**
   * Small utility to <b>lazily</b> run an "initialisation" function for the target instances of the
   * bound {@link Lift}.
   *
  ...
  public static class LiftBinder<T> extends TypedBinder<Lift<T>> {
    ...
    public <R> void by(Instance<? extends R> related, BiConsumer<T, R> initFunction) {
      to(
          (target, as, context) -> {
            initFunction.accept(target, context.resolve(dependency(related).injectingInto(as)));
            return target;
          });
    }

    public final <R> void forEach(Class<? extends R> related, BiConsumer<T, R> initFunction) {
      forEach(raw(related), initFunction);
    }

    public <R> void forEach(Type<? extends R> related, BiConsumer<T, R> initFunction) {
      Dependency<? extends R[]> dep = dependency(related.addArrayDimension().asUpperBound());
      to(
          (target, as, context) -> {
            for (R arg : context.resolve(dep.injectingInto(as))) initFunction.accept(target, arg);
            return target;
          });
    }
  }
  ...
  /**
   * Small utility to <b>eagerly</b> run an initialisation function for a group instances managed by
   * the {@link Injector} at the end of the bootstrapping process.
   *
   * <p>The basic principle is that the {@link #target} {@link Instance} is initialised on the basis
   * of some other dependency instance that is resolved during initialisation phase and provided to
   * the {@link BiConsumer} function. This can be one or many of such instances.
   *
   * <p>In contrast to a plain {@link Lift} that runs <b>lazily</b> when an instance of the matching
   * type is constructed this initialisation is performed directly after the {@link Injector}
   * context is created.
   *
   * @param <T> type of the instances that should be setup during bootstrapping
   * @since 8.1
   */
  public static class BootBinder<T> {
    ...
    public <R> void forEach(Type<? extends R> related, BiConsumer<T, R> initFunction) {
      Dependency<? extends R[]> dep =
          dependency(related.addArrayDimension().asUpperBound()) //
      ...
      binder
          ...
          .to(
              (impl, as, context) -> {
                ...
                for (R arg : context.resolve(dep)) initFunction.accept(obj, arg);
                return impl;
              });
    }

    public final <R> void by(Class<? extends R> related, BiConsumer<T, R> initFunction) {
      by(defaultInstanceOf(raw(related)), initFunction);
    }

    public final <R> void by(
        Name relatedName, Class<? extends R> relatedType, BiConsumer<T, R> initFunction) {
    ...
    public <R> void by(Instance<? extends R> related, BiConsumer<T, R> initFunction) {
      binder
          ...
          .to(
              (impl, as, context) -> {
                T obj = context.resolve(target);
                R arg = context.resolve(dependency(related).injectingInto(as));
  ...
  public static class PluginBinder<T> {
    ...
    public void into(Class<?> pluginPoint, String property) {
      binder.multibind(pluginPoint(pluginPoint, property), raw(Class.class)).to(plugin);
      if (isClassConstructable(plugin)) binder.implicit().construct(plugin);
  ...
  /**
   * Connecting is the dynamic process of identifying methods in target types that should be subject
   * to a {@link Connector} referenced by name.
   *
   * <p>The {@link Connector} is expected to be bound explicitly elsewhere.
   *
   * @since 8.1
   */
  public static class ConnectBinder {
     ...
     *
     * @param target can be understood as the scope in which connecting applies to identified
     *     methods.
  ...
  /**
   * @param <T> type of the class(es) (includes subtypes) that are subject to connecting
   * @since 8.1
   */
  public static final class ConnectTargetBinder<T> implements ConnectorBinder {
    ...
    public ConnectTargetBinder(Binder binder, ProducesBy connectsBy, Type<T> target) {
      this.binder = binder;
      this.connectsBy = connectsBy;
      this.target = target;
    }
    ...
    private T connect(Name connectorName, T instance, Type<?> as, Injector context) {
      ...
      if (connected != null && connected.length > 0) {
        ...
        for (Method m : connected) connector.connect(instance, as, m);
      }
      return instance;
    }
  }
  ...
  /**
   * The {@link AutoBinder} makes use of mirrors to select and bind constructors for beans and
   * methods as factories and {@link Name} these instances as well as provide {@link Hint}s.
   *
   * @since 8.1
   */
  public static class AutoBinder {
    ...
    protected AutoBinder(RootBinder binder, Name scope) {
      this(binder.on(binder.bind().asPublished()).on(binder.bind().next()), env(binder, scope));
    }

    private static Env env(RootBinder binder, Name scope) {
      Env env = binder.env().withIsolate();
      return scope.equalTo(Scope.mirror) ? env : env.with(ScopesBy.class, target -> scope);
    }
    ...
    private void in(Class<?> impl, Object instance, Hint<?>... construction) {
      boolean needsInstance1 = bindProducesIn(impl, instance);
      boolean needsInstance2 = bindAccessesIn(impl, instance);
      if (!needsInstance1 && !needsInstance2) return; // do not try to construct the class
      if (instance != null && !(instance instanceof Hint))
        return; // if there is an instance don't bind constructor unless it is just a Hint
      Constructor<?> target =
          env.property(ConstructsBy.class).reflect(impl.getDeclaredConstructors(), construction);
      if (target != null) toConstructor(Scope.auto, target, construction);
    }
    ...
    private boolean bindAccessesIn(Class<?> impl, Object instance) {
      ...
      for (Field constant : constants) {
        if (asConstant(constant, instance)) needsInstance |= !isStatic(constant.getModifiers());
      }
    ...
    /**
     * This method will not make sure an instance of the {@link Method}'s declaring class is created
     * if needed. This must be bound elsewhere.
     *
     * @param target a method that is meant to create instances of the method return type
     * @param instance can be null to resolve the instance from {@link Injector} context later (if
     *     needed). It can also be a {@link Hint} to that instance. {@code null} for {@code static}
     *     methods.
     * @param args optional method argument {@link Hint}s
     * @return true if a target was bound, else false (this is e.g. the case when the {@link Method}
     *     returns void)
     */
    public boolean asProducer(Method target, Object instance, Hint<?>... args) {
      ...
      binder
          .per(scope == null ? Scope.auto : scope) //
    ...
    public <T> void toConstructor(Name scope, Constructor<T> target, Hint<?>... hints) {
      if (target == null) throw InconsistentBinding.generic("Provided Constructor was null");
      ...
      if (name.isDefault()) {
        scopedBinder
            .withPublishedAccess(env.property(PublishesBy.class))
            .bind(impl)
            // TODO use actual type hint/ref
            .expand(constructs(raw(impl), target, env, hints));
      } else {
        for (Type<? super T> st : raw(impl).supertypes())
          if (st.isInterface()) scopedBinder.implicit().bind(name, st).to(name, impl);
      }
    }
  }
  ...
  public static class ScopedBinder extends TargetedBinder
      implements ConfiguringInstanceLocalBinder<TargetedBinder> {
  ...
  public static class TargetedBinder extends Binder
      implements ParentLocalBinder<TargetedBinder>, PackageLocalBinder<Binder> {
  ...
  public static class TypedBinder<T> implements ReferenceBinder<T>, SupplierBinder<T> {
    ...
    protected final void expand(Descriptor value) {
      declareBindingsIn(
          bind() //
              .asType(locator, BindingType.VALUE, null),
          value);
    }
    ...
    /**
     * By default constants are not scoped. This implies that no initialisation occurs for constants
     * as they are directly returned from the {@link Generator} which simply holds the returned
     * constant value.
     *
     * <p>In contrast to {@link #to(Object)} a scoped constant exist within a {@link Scope} like
     * instances created by the container. This has the "side-effect" that {@link Lift}
    ...
    public final void toMultiple(T constant1, T... constants) {
      ...
      for (T constant : constants) multibinder.toConstant(constant);
    }
    ...
    public void toConstructor(Class<? extends T> impl, Hint<?>... hints) {
      if (!isClassConstructable(impl)) throw InconsistentDeclaration.notConstructable(impl);
      Constructor<? extends T> target =
          (Constructor<? extends T>)
              env(ConstructsBy.class) //
                  .reflect(impl.getDeclaredConstructors(), hints);
      if (target == null)
        throw InconsistentBinding.generic("No usable Constructor for type: " + impl);
   ...
   * all known instances of the element type will no longer be used whenever the bind made applies.
   */
  public static class TypedElementBinder<E> extends TypedBinder<E[]> {
    ...
    private E[] array(Object... elements) {
      ...
      if (elements.getClass() == rawType) return (E[]) elements;
      Object[] res = newArray(getType().baseType().rawType, elements.length);
