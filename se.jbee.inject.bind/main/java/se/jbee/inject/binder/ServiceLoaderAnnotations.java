...
/**
 * A {@link Module} that is meant to be installed during the bootstrapping of a custom {@link Env}.
 * It loads all {@link ModuleWith} from {@link ServiceLoader} which define the effect of an
 * particular type level {@link Annotation}. That {@link Annotation} is either referred to by
 * annotated the {@link ModuleWith} implementation class with the {@link Extends} annotation (use
 * {@link Extends#value()}) or by annotated the implementation class with the target {@link
 * Annotation} itself. In that case that {@link Annotation} must be the only runtime annotation
 * present.
 *
 * @since 8.1
 */
public class ServiceLoaderAnnotations extends EnvModule {
  ...
  protected final void bind(Class<? extends Annotation> name, ModuleWith<Class<?>> value) {
