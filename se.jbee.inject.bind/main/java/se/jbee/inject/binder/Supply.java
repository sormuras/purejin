...
public final class Supply {
  ...
  public static final Supplier<Provider<?>> PROVIDER = (dep, context) //
      -> byLazyProvider(dep.onTypeParameter().uninject().ignoredScoping(), context);

  private static final Supplier<?> REQUIRED =
      (dep, context) -> {
   ...
   *
   * <p>Basically we just resolve the array of the element type (generic of the list). Arrays itself
  ...
  /**
   * A {@link Supplier} used as fall-back. Should a required {@link Locator} not be provided it is
   * still bound to this supplier that will throw an exception should it ever be called.
   */
  ...
  public static <T> Supplier<T> bySupplierReference(Class<? extends Supplier<? extends T>> type) {
    return (dep, context) ->
        context
            .resolve(dep.onInstance(anyOf(type))) //
            .supply(dep, context);
  }

  public static <E> Supplier<E[]> byElementReferences(
      Type<E[]> arrayType, Hint<? extends E>[] elements) {
  ...
  public static <T> Supplier<T> byDependencyReference(Dependency<T> dependency) {
    return (dep, context) -> context.resolve(dependency);
  }

  /** E.g. used to "forward" a {@link Collection} to {@link List} of same element type. */
  public static <T> Supplier<T> byParameterizedInstanceReference(Instance<T> instance) {
    return (dep, context) -> {
  ...
  /**
   * This effectively avoid redoing {@link Resource} resolution for each invocation. Instead the
   * {@link Resource} is resolved once and continuously used from there on to {@link
   * Resource#generate(Dependency)} the values.
   */
  public static <T> Provider<T> byLazyProvider(Dependency<T> dep, Injector injector) {
    if (dep.type().arrayDimensions() == 1) return () -> injector.resolve(dep);
    @SuppressWarnings("unchecked")
    Resource<? extends T> resource =
        injector.resolve(dep.typed(raw(Resource.class).parameterized(dep.type())));
  ...
  public interface ArrayBridge<T> extends Supplier<T> {
    ...
    default T supply(Dependency<? super T> dep, Injector context) {
      return bridge(context.resolve(dep.typed(dep.type().parameter(0).addArrayDimension())));
    }
   ...
   */
  private static final class ArrayElementReferencesSupplier<E> extends WithArgs<E[]> {

    private final Type<E[]> arrayType;
    private final Hint<? extends E>[] elements;

    ArrayElementReferencesSupplier(Type<E[]> arrayType, Hint<? extends E>[] elements) {
  ...
  private static final class Construct<T> extends WithArgs<T> implements Annotated {
    ...
    protected T invoke(Object[] args, Injector context) {
      try {
        if (newInstance == null)
          newInstance =
              context.resolve(
                  dependency(New.class).injectingInto(constructs.target.getDeclaringClass()));
  ...
  private static final class Produce<T> extends WithArgs<T> implements Annotated {

    private Object instance;
    private Invoke invoke;
    private final Produces<T> produces;
    private final Class<T> returns;
    private final Map<java.lang.reflect.TypeVariable<?>, UnaryOperator<Type<?>>>
    ...
    Produce(Produces<T> produces) {
      ...
      this.typeVariableResolvers =
          produces.isGeneric()
              ? TypeVariable.typeVariables(produces.target.getGenericReturnType())
              : null;
    }
    ...
    protected T invoke(Object[] args, Injector context) {
      ...
      if (invoke == null)
        invoke =
            context.resolve(
                dependency(Invoke.class).injectingInto(produces.target.getDeclaringClass()));
      try {
    ...
    @Override
    protected Hint<?>[] actualParametersFor(Dependency<? super T> dep, Injector context) {
      // TODO it is not always sure that produces.actualDeclaringType() is the actual owner object
      // type => look at dependency!
      Hint<?>[] actualParameters =
          produces.actualParameters(produces.actualDeclaringType(), context);
      if (!produces.isGeneric()) return actualParameters;
      Hint<?>[] actualTypeHints = actualParameters.clone();
      Map<java.lang.reflect.TypeVariable<?>, Type<?>> actualTypes =
          TypeVariable.actualTypesFor(typeVariableResolvers, dep.type());
  ...
  public abstract static class WithArgs<T> implements Supplier<T> {
    ...
    @Override
    public T supply(Dependency<? super T> dep, Injector context) throws UnresolvableDependency {
      ...
      if (local == null || !local.site.equalTo(dep)) {
        local = new InjectionSite(context, dep, actualParametersFor(dep, context));
        previous = local;
      }
