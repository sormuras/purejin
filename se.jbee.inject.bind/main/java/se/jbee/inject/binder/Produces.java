...
/**
 * A {@link Produces} is the {@link ValueBinder} expansion wrapper for a method bound to a
 * particular instance (if not static) that produces instances to inject.
 *
 * @param <T> type of the value yield by the factory method
 */
public final class Produces<T> extends ReflectiveDescriptor<Method, T> {

  public static <T> Produces<? extends T> produces(
      Type<T> expectedType, Object owner, Method target, HintsBy strategy, Hint<?>... args) {
    ...
    return new Produces<>(expectedType, actualType, owner, target, strategy, args);
  }

  public static <T> Produces<?> produces(
      Object owner, Method target, HintsBy strategy, Hint<?>... explicitHints) {
    @SuppressWarnings("unchecked")
    Type<T> actualType = (Type<T>) actualType(owner, target);
    checkBasicCompatibility(Type.returnType(target), actualType, target);
    return new Produces<>(actualType, actualType, owner, target, strategy, explicitHints);
  }
  ...
  private static Type<?> actualType(Object owner, Method target) {
    return requiresActualReturnType(
            target, Method::getReturnType, Method::getAnnotatedReturnType) //
  ...
  /**
   * @return true, if the called {@link Method} uses type variables that need replacement from the
   *     actual call {@link se.jbee.inject.Dependency}.
   */
  ...
  /**
   * If a {@link Method} has a {@link Type} parameter as its first parameter we assume it is meant
   * to be the actual type for a type variable used by the {@link #target} method.
   *
   * @return true if the target {@link Method} has a matching {@link Type} parameter as it first
   *     parameter, otherwise false
   */
  public boolean isGenericTypeAware() {
    if (!isGeneric()) return false;
    Type<?> arg0Type = parameterType(target.getParameters()[0]);
    if (arg0Type.rawType != Type.class) return false;
  ...
  public Hint<?>[] actualParameters(Type<?> actualDeclaringType, Injector context) {
    ...
    if (isGenericTypeAware()) {
      // use a constant null hint to blank first parameter as it is filled in with actual type on
      // method invocation
      Hint<?> actualTypeHint = Hint.constantNull(Type.parameterType(target.getParameters()[0]));
