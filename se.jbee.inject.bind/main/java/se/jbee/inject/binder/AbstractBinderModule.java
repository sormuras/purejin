...
/**
 * A {@link RootBinder} that can be initialized using the {@link #__init__(Env, Bindings)} method.
 *
 * <p>This allows to change the start {@link Bind} once.
 */
public abstract class AbstractBinderModule extends RootBinder implements Bundle {
  ...
  protected static void installAnnotated(Class<? extends Bundle> bundle, Bootstrapper bootstrap) {
      ...
      if (installs.features() != Enum.class) {
        ...
        if (!raw(features).isAssignableTo(raw(Dependent.class)))
          throw new InconsistentDeclaration(
              "@Installs#features() Class must be compatible with <F extends Enum<F> &"
                  + " Toggled<F>>");
        Class<? extends Annotation> by = installs.by();
        Annotation selection = by == Annotation.class ? null : bundle.getAnnotation(by);
  ...
  private static <E extends Enum<E> & Dependent<E>> void install(
    ...
    if (selection == null) {
      ...
      try {
        Object[] value = (Object[]) selection.getClass().getMethod("value").invoke(selection);
        if (value.length == 0) return;
        if (value[0].getClass() != features)
          throw new InconsistentDeclaration(
              "The annotation referenced by @Installs#by() must have the same component enum type"
                  + " as @Installs#s#features()");
