...
abstract class ReflectiveDescriptor<M extends AnnotatedElement & Member, T>
  ...
  protected ReflectiveDescriptor(
      Type<? super T> expectedType,
      Type<T> actualType,
      Object as,
      M target,
      HintsBy strategy,
  ...
  static <T extends Member> boolean requiresActualReturnType(
      T member, Function<T, Class<?>> type, Function<T, AnnotatedType> genericType) {
  ...
  static Type<?> actualDeclaringType(Object as, Member target) {
    return as instanceof Hint ? ((Hint<?>) as).asType : Type.classType(target.getDeclaringClass());
  }

  static <T> void checkBasicCompatibility(Type<?> memberType, Type<T> expectedType, Member target) {
    if (memberType.equalTo(Type.WILDCARD)) return; // it could be any type - fine
    if (memberType.isAssignableTo(expectedType)) return; // a supertype of what we have - fine
  ...
  protected final void checkConsistentExplicitHints(Parameter[] params) {
    if (explicitHints.length == 0) return;
    Type<?>[] types =
        arrayMap(params, Type.class, p -> actualParameterType(p, actualDeclaringType()));
    Hint<?>[] hints = new Hint[params.length];
    for (Hint<?> hint : explicitHints) {
      int i = Hint.indexForType(types, hint, hints);
      if (i < 0) throw InconsistentDeclaration.incomprehensibleHint(hint);
      hints[i] = hint;
    }
  }
  ...
  /**
   * @return The actual {@link Method#getDeclaringClass()} when considering potential type arguments
   *     for type parameters as specified by the {@link Hint} given for the owner.
   * @see #isHinted()
   */
  ...
  /**
   * @return By convention if the given owner is a {@link Hint} object this is not considered the
   *     actual target object but as a reference to resolve the actual target. This is specifically
   *     useful when the target is a type with type parameters as there is no other way to know for
   *     what actual type parameters the instance was build for then to resolve it for a specific
