...
/**
 * Picks the {@link Field}s that are bound as constants for the {@link Field}'s {@link Type}.
 *
 * <p>This is mainly used to allow declaring the bound {@link Type} simply by making a field of that
 * type. This way the {@link Type} is automatically constructed and the user can use the normal Java
 * language to define the field. Also this makes sure the constant of the field value is assignable
 * to the bound type.
 *
 * @since 8.1
 */
...
public interface AccessesBy {
  ...
  /**
   * @return The {@link Field}s that should be used in the context this {@link AccessesBy} is used.
   *     Return {@code null} when no decision has been made. Returns empty array when the decision
   *     is to not share any fields.
   */
  ...
  static AccessesBy fields(
      Function<Class<?>, Field[]> pool, Predicate<Field> filter, boolean includeInherited) {
    return fields(pool, filter, impl -> includeInherited ? Object.class : impl.getSuperclass());
  }

  static AccessesBy fields(
      Function<Class<?>, Field[]> pool, Predicate<Field> filter, UnaryOperator<Class<?>> end) {
    return impl -> arrayFilter(impl, end.apply(impl), pool, filter).toArray(new Field[0]);
  }
  ...
  default AccessesBy ignoreGenericType() {
    return select(field -> !(field.getGenericType() instanceof TypeVariable<?>));
  }
  ...
  default AccessesBy in(Packages filter) {
    return impl -> filter.contains(raw(impl)) ? this.reflect(impl) : null;
  }
}
