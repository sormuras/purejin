...
public interface ConstructsBy {
   ...
   *     object of the given type. In case one with parameters is returned the these are solved
   *     (injected).
   *     <p>Returns {@code null} when no suitable constructor was found.
   */
  Constructor<?> reflect(Constructor<?>[] candidates, Hint<?>... hints);

  /**
   * Simply returns the first candidate or null. This is a good root when using {@link
   * #sortedBy(Comparator)}.
   */
  ConstructsBy FIRST = (candidates, hints) -> candidates.length == 0 ? null : candidates[0];

  /**
   * Is the {@link Constructor} that is most visible and out of those has the most parameters but
   * does not have a parameter that is the type itself (as this would usually cause an endless loop
   * when trying to inject it).
   *
   * <p>The approach is to first sort the possible constructor accordingly, remove problematic ones
   * and go with the first of the resulting candidates.
   */
  ConstructsBy OPTIMISTIC =
      FIRST
  ...
  default ConstructsBy select(Predicate<Constructor<?>> filter) {
    return (candidates, hints) ->
        candidates.length == 0 ? null : reflect(arrayFilter(candidates, filter), hints);
  }
