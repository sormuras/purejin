...
/**
 * Extracts the relevant {@link Method}s for a given target {@link Class}. These can be methods for
 * different purposes. For example factory methods or methods that are used in advanced abstractions
 * like actions.
 *
 * @since 8.1
 */
...
public interface ProducesBy {
  ...
  /**
   * The default pool of methods contains all non synthetic {@link Method} including all inherited
   * ones except for those declared (and not overridden) in {@link Object}.
   */
  ProducesBy OPTIMISTIC = declaredMethods(m -> !m.isSynthetic(), true);

  /**
   * @return The {@link Method}s that should be used in the context this {@link ProducesBy} is used.
   *     Return {@code null} when no decision has been made. Returns empty array when the decision
   *     is to not use any producer methods.
   */
  ...
  static ProducesBy declaredMethods(Predicate<Method> filter, boolean includeInherited) {
    return methods(Class::getDeclaredMethods, filter, includeInherited);
  }

  static ProducesBy methods(
      Function<Class<?>, Method[]> pool, Predicate<Method> filter, boolean includeInherited) {
    return methods(pool, filter, impl -> includeInherited ? Object.class : impl.getSuperclass());
  }

  static ProducesBy methods(
      Function<Class<?>, Method[]> pool, Predicate<Method> filter, UnaryOperator<Class<?>> end) {
    return impl -> arrayFilter(impl, end.apply(impl), pool, filter).toArray(new Method[0]);
  }
  ...
  default ProducesBy ignoreGenericReturnType() {
    return select(method -> !(method.getGenericReturnType() instanceof TypeVariable<?>));
  }
  ...
  default ProducesBy in(Packages filter) {
    return impl -> filter.contains(raw(impl)) ? reflect(impl) : null;
  }

  default ProducesBy in(Class<?> api) {
    return select(
        method ->
            raw(method.getDeclaringClass()) //
