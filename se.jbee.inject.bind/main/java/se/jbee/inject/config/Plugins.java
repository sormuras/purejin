...
/**
 * {@link Plugins} are an {@link Extension} that makes resolving plugged {@link Class}es for
 * specific plugin points more convenient and formalise the convention of the mechanism.
 *
 * @since 8.1
 */
public final class Plugins implements ContextAware<Plugins>, Extension {
   ...
   * @param property property name used as sub-space, use {@link Class#getCanonicalName()} in case
   *     no specific property should be targeted.
   * @return A fully qualified plugin point name
   */
  public static Name pluginPoint(Class<?> point, String property) {
    return property.isEmpty() ? Name.named(point) : Name.named(property).in(Name.named(point));
  }
  ...
  public Plugins inContext(Dependency<? super Plugins> context) {
    if (context.isNotTargeted()) return target == null ? this : targeting(null);
  ...
  public Class<?>[] forPoint(Class<?> point, String property) {
    ...
    Dependency<Class[]> plugins =
        dependency(raw(Class[].class)).named(pluginPoint(point, property));
    return context.resolve(target == null ? plugins : plugins.injectingInto(target));
  }
