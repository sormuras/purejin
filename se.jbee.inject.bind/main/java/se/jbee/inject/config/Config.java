...
/**
 * The {@link Config} is a runtime-configuration of the application. It allows other "external"
 * software modules to contribute to the configuration of other software components.
 *
 * <p>A {@link Config} is an {@link Extension} that uses the {@link Dependency} hierarchy to
 * effectively name-space configuration values to avoid name collisions.
 *
 * <p>As it is {@link ContextAware} it can be injected into the configured bean already being scoped
 * to the target, that means only the configurations for the target instance are "in scope" or
 * visible.
 *
 * @since 8.1
 */
public class Config implements ContextAware<Config>, Extension {
  ...
  /** If {@link Config} is injected into other resources the name-space is automatically set */
  @Override
  public Config inContext(Dependency<? super Config> context) {
    if (context.isNotTargeted()) return ns == null ? this : new Config(this.context, null);
    Instance<?> target = context.target();
    if (target.name.isAny()) return of(target.type.rawType);
    return of(target);
  }
  ...
  public final class Value<A> {
    ...
    public <B> Optional<B> as(Type<B> type) {
      if (from.isAssignableTo(type)) return (Optional<B>) optionalValue(from, property);
      Converter<A, B> converter =
          orElse(null, () -> Config.this.context.resolve(Converter.converterTypeOf(from, type)));
      if (converter == null) return empty();
      return Config.this
          .optionalValue(from, property) //
