...
/**
 * Utility to create an {@link Injector} or {@link Env} context from {@link Bundle}s and {@link
 * Module}s.
 */
public final class Bootstrap {
  ...
  /**
   * @return The {@link Env} purely defined by {@link Bundle}s provided via {@link ServiceLoader}.
   *     Once created this method always returns the same {@link Env} instance.
   */
  ...
  /**
   * @return The {@link Injector} purely defined by {@link Bundle}s provided via {@link
   *     ServiceLoader}. The bootstrapping always uses the {@link #currentEnv()}. Once created this
   *     method always returns the same {@link Injector} instance.
   */
  ...
  /**
   * @return The {@link Injector} context purely created from {@link Bundle}s found using {@link
  ...
  /**
   * @return The {@link Env} context purely created from {@link Bundle}s found using {@link
   *     ServiceLoader}. These need to have the {@link se.jbee.inject.Extends} annotation referring
   *     to the {@link Env} class.
   */
  ...
  @SafeVarargs
  public static Injector injector(Env env, Bindings bindings, Class<? extends Bundle>... roots) {
  ...
  public static Injector injector(Env env, Class<? extends Bundle> root, Bindings bindings) {
    return injector(env, bindings, modules(env).installedModules(root));
  }

  private static Injector injector(Env env, Bindings bindings, Module[] modules) {
    return Container.injector(
        env.property(BindingConsolidation.class) //
            .consolidate(env, (bindings.declaredFrom(env, modules))));
  }
  ...
  private static final class BuiltinBootstrapper
    ...
    private final Map<Class<? extends Bundle>, Set<Class<? extends Bundle>>> bundleChildren =
        new IdentityHashMap<>();
    private final Map<Class<? extends Bundle>, List<Module>> bundleModules =
        new IdentityHashMap<>();
    ...
    public void install(Class<? extends Bundle> bundle) {
      if (uninstalled.contains(bundle) || installed.contains(bundle)) return;
      ...
      if (!stack.isEmpty()) {
        ...
        bundleChildren.computeIfAbsent(parent, key -> new LinkedHashSet<>()).add(bundle);
      }
      stack.push(bundle);
      createBundle(bundle).bootstrap(this);
      if (stack.pop() != bundle) throw new IllegalStateException(bundle.getCanonicalName());
    }
    ...
    private <T> T createBundle(Class<T> bundle) {
      ...
      try {
        ...
        throw new InconsistentDeclaration("Failed to create bundle: " + bundle, e);
      }
    }
    ...
    public <F extends Enum<F>> void install(
      ...
      if (!edition.featured(bundle)) return;
      createBundle(bundle)
          .bootstrap(
              (actual, bundleForFlag) -> {
    ...
    public final <F extends Enum<F> & Dependent<F>> void install(F... elements) {
      if (elements.length > 0) {
        ...
        if (!edition.featured(e0.getClass())) return;
        final EnumSet<F> installing = EnumSet.of(e0, elements);
        e0.bootstrap(
            (on, bundle) -> {
              if (installing.contains(on)) BuiltinBootstrapper.this.install(bundle);
            });
      }
    }
    ...
    @Override
    @SafeVarargs
    public final <F extends Enum<F> & Dependent<F>> void uninstall(F... elements) {
      if (elements.length > 0) {
        final EnumSet<F> uninstalling = EnumSet.of(elements[0], elements);
        elements[0].bootstrap(
            (on, bundle) -> {
              if (uninstalling.contains(on)) uninstall(bundle);
            });
      }
    }
    ...
    public void install(Module module) {
      ...
      if (uninstalled.contains(bundle) || !edition.featured(module.getClass())) return;
      bundleModules
          .computeIfAbsent(
              bundle, //
              key -> new ArrayList<>())
    ...
    final Class<? extends Bundle>[] bundleAll(Class<? extends Bundle>... roots) {
      ...
      for (Class<? extends Bundle> root : roots) addAllInstalledIn(root, newlyInstalled);
    ...
    final Module[] modulesOf(Class<? extends Bundle>[] bundles) {
      ...
      for (Class<? extends Bundle> b : bundles) {
        ...
        if (modules != null) newlyInstalled.addAll(modules);
      }
    ...
    public void uninstall(Class<? extends Bundle> bundle) {
      if (uninstalled.contains(bundle)) return;
      uninstalled.add(bundle);
      installed.remove(bundle);
      for (Set<Class<? extends Bundle>> c : bundleChildren.values()) c.remove(bundle);
      bundleModules.remove(bundle); // we are sure we don't need its modules
    }

    private void addAllInstalledIn(
        Class<? extends Bundle> bundle, Set<Class<? extends Bundle>> accu) {
      accu.add(bundle);
      Set<Class<? extends Bundle>> children = bundleChildren.get(bundle);
      if (children == null) return;
      for (Class<? extends Bundle> c : children) if (!accu.contains(c)) addAllInstalledIn(c, accu);
    }
  }
}
