...
/**
 * The {@link se.jbee.inject.bind.Module} that setups the values required in an {@link Env} used
 * with the {@link Binder} API.
 */
public class DefaultEnv extends SimpleModule {

  public static Env bootstrap() {
    return Container.injector(
            Bindings.newBindings().declaredFrom(DefaultEnv::property, new DefaultEnv()))
        .asEnv();
  }
   ...
   *
   * <p>It simply does not know anything and throws an {@link InconsistentDeclaration}.
   */
