...
public enum DefaultFeature implements Dependent<DefaultFeature> {
  ...
  LOGGER(false),
  /**
   * Adds: Support for injection of {@link Optional}s. If {@link Optional} parameters cannot be
   * resolved {@link Optional#empty()} is injected.
   */
  OPTIONAL(false),
  /**
   * Adds: That primitive arrays can be resolved for their wrapper types.
   *
   * <p>Note that this only supports one-dimensional arrays of int, long, float, double and boolean.
   * For further support add similar suppliers following the example given.
   */
  PRIMITIVE_ARRAYS(false),
  /** Adds: Support for {@link Injector} sub-contexts. */
  SUB_CONTEXT(true),
  /**
   * Adds: Binds the bootstrapping {@link Env} as the {@link Name#DEFAULT} {@link Env} in the {@link
   * Injector} context.
   */
  ENV(true),
  ...
  public static final DefaultFeature[] INSTALLED_BY_DEFAULT =
      Arrays.stream(DefaultFeature.values()) //
  ...
  @Override
  public void bootstrap(Bootstrapper.DependentBootstrapper<DefaultFeature> bootstrapper) {
  ...
  private static class OptionalBridgeModule extends BinderModule {
    ...
    <T> Optional<T> optional(Dependency<? super Optional<T>> dep, Injector context) {
      try {
        return Optional.ofNullable((T) context.resolve(dep.onTypeParameter().uninject()));
  ...
  /**
   * Provides {@link Injector} sub-contexts on the basis of {@link Plugins}s. When installing a
   * {@link Bundle} in a sub-context this simply plugs that {@link Bundle} into the point of the
   * sub-context which is {@link Injector} and the sub-context {@link Name}.
   *
   * <p>Extracting the {@link Bundle}s via {@link Plugins} is one part. The creation of the {@link
   * Injector} from those {@link Bundle}s is extracted into a separate {@link Function} so this can
  ...
  private static final class SubContextModule extends BinderModule
    ...
    protected void declare() {
      asDefault()
          .bind(functionTypeOf(Class[].class, Injector.class)) //
    ...
    @Override
    @SuppressWarnings("unchecked")
    public Injector supply(Dependency<? super Injector> dep, Injector context)
        throws UnresolvableDependency {
      @SuppressWarnings("unchecked")
      Class<? extends Bundle>[] bundles =
          (Class<? extends Bundle>[]) //
              context.resolve(Plugins.class).forPoint(Injector.class, dep.instance.name.toString());
      if (bundles.length == 0)
        return this; // this module acts as an Injector that directly fails to resolve any
      // Dependency
    ...
    public <T> T resolve(Dependency<T> dep) throws UnresolvableDependency {
      throw new UnresolvableDependency.ResourceResolutionFailed("Empty SubContext Injector", dep);
    }
  }
  ...
  private static final class ObtainableModule extends BinderModule {
    ...
    static final class ObtainableCollection<T, E> implements Obtainable<T> {
      ...
      @SuppressWarnings("unchecked")
      private T resolve() {
        Resource<E>[] resources = context.resolve(dep.typed(resourcesTypeOf(dep.type())));
    ...
    static final class ObtainableInstance<T> implements Obtainable<T> {
      ...
      public <X extends Exception> T orElseThrow(
          Function<UnresolvableDependency, ? extends X> exceptionTransformer) throws X {
        T res = obtain();
  ...
  private static final class SelfModule extends BinderModule {
    ...
    private static Type<?> supplyType(Dependency<? super Type<?>> dep, Injector context) {
      return dep.injection(findTypeInjectionFrame(dep)).dependency.type;
    }

    private static Name supplyName(Dependency<? super Name> dep, Injector context) {
    ...
    private static int findTypeInjectionFrame(Dependency<?> dep) {
      ...
      if (!target.isRawType()) return 1;
      for (int frame = 2; frame < dep.injectionDepth(); frame++) {
        Type<?> candidate = dep.injection(frame).dependency.type;
        if (candidate.rawType == target.rawType && !candidate.isRawType()) return frame;
      }
      return 1;
    }
  }
  ...
  private static final class PrimitiveArraysModule extends BinderModule {
    ...
    private static <T> T copyToPrimitiveArray(Object[] src, T dest) {
      for (int i = 0; i < src.length; i++) Array.set(dest, i, src[i]);
      return dest;
    }

    private static int[] ints(Dependency<? super int[]> dep, Injector context) {
      Integer[] wrappers = context.resolve(dep.typed(raw(Integer[].class)));
      return copyToPrimitiveArray(wrappers, new int[wrappers.length]);
    }

    public static long[] longs(Dependency<? super long[]> dep, Injector context) {
      Long[] wrappers = context.resolve(dep.typed(raw(Long[].class)));
      return copyToPrimitiveArray(wrappers, new long[wrappers.length]);
    }

    public static float[] floats(Dependency<? super float[]> dep, Injector context) {
      Float[] wrappers = context.resolve(dep.typed(raw(Float[].class)));
      return copyToPrimitiveArray(wrappers, new float[wrappers.length]);
    }

    public static double[] doubles(Dependency<? super double[]> dep, Injector context) {
    ...
    public static boolean[] booleans(Dependency<? super boolean[]> dep, Injector context) {
      Boolean[] wrappers = context.resolve(dep.typed(raw(Boolean[].class)));
