 ...
 *
 * <p>It does not implement the interface since it has no state. Use a method reference lambda on
 * {@link #consolidate(Env, Binding[])} instead.
 *
 * @since 8.1
 */
public final class DefaultBindingConsolidation {
  ...
  /**
   * Removes those bindings that are ambiguous but also do not clash because of different {@link
   * DeclarationType}s that replace each other.
   */
  public static Binding<?>[] consolidate(Env env, Binding<?>[] bindings) {
    ...
    for (int i = 1; i < bindings.length; i++) {
      ...
      final boolean equalResource = lastUnique.signature.equalTo(current.signature);
      ...
      if (curType == DeclarationType.REQUIRED) {
        ...
        if (!isDuplicateIdenticalConstant(true, lastUnique, current) && i - 1 == lastUniqueIndex) {
        ...
        if (current.source.declarationType == DeclarationType.MULTI
            && isDuplicateIdenticalConstant(equalResource, lastUnique, current)) {
          dropped.add(current);
        } else {
          uniques.add(current);
          lastUniqueIndex = i;
  ...
  private static boolean isDuplicateIdenticalConstant(
      boolean equalResource, Binding<?> lastUnique, Binding<?> current) {
  ...
  private static Binding<?>[] withoutProvidedThatAreNotRequiredIn(
      Env env, List<Binding<?>> bindings, Set<Type<?>> required, List<Binding<?>> dropped) {
    ...
    if (!required.isEmpty()) throw new ResourceResolutionFailed(required, dropped);
  ...
  /**
   * This must look very confusing at first. What we want to do here is add a {@link Binding} that
   * has all the bindings as array including itself. But this must be inserted at the correct
   * sorting position. Because of the hen-egg situation with the array itself we use {@link
   * AtomicReference} as a box that we can fill later.
   */
  private static Binding<?>[] withListItselfInserted(List<Binding<?>> res) {
    AtomicReference<Binding<?>[]> box = new AtomicReference<>();
    Binding<?> self =
        Binding.binding(
            Locator.locator(Binding[].class),
            BindingType.PREDEFINED,
            (dep, context) -> box.get(),
            Scope.container,
            Source.source(Binding.class));
