...
/**
 * A {@link Scope} that maintains a map of instances where the key is derived from the {@link
 * Dependency} itself. In the simplest case this is the {@link #typeSignature(Dependency)} or the
 * {@link #instanceSignature(Dependency)} but it can also include the injection hierarchy as used by
 * {@link #hierarchicalInstanceSignature(Dependency)}.
 *
 * <p>One use case are {@link #JVM} singletons. As the scope is kept in a normal constant that is
 * shared within the JVM this effectively shares the map's instances within the same JVM.
 *
 * @since 8.1
 */
public final class TypeDependentScope implements Scope {
  ...
  public static String targetInstanceSignature(Dependency<?> dep) {
    ...
    for (int i = dep.injectionDepth() - 1; i >= 0; i--) b.append(dep.target(i));
    return b.toString();
  }

  /**
   * Cannot use a {@link ConcurrentHashMap} for this since it has the property of not allowing
   * updates to any other key while an entry being updated atomically but resolving dependencies
   * during the update (in the update function) could very well lead to initialising other entries.
   */
  ...
  public <T> T provide(int serialID, int resources, Dependency<? super T> dep, Provider<T> provider)
    ...
    return (T) instances.computeIfAbsent(injectionKey.apply(dep), k -> provider.provide());
  }
}
