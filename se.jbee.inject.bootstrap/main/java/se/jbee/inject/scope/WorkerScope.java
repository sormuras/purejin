...
/** A {@link Scope} that is linked to the current {@link Thread} using the {@link Controller}. */
public final class WorkerScope implements Scope {

  // TODO cleanup that checks if thread is alive => general feature to allow hook in for cleanup
  // without needed to explicitly work with scheduler
  ...
  public <T> T provide(int serialID, int resources, Dependency<? super T> dep, Provider<T> provider)
    ...
    if (state == null) {
      throw new UnresolvableDependency.SupplyFailed("Scope error", contextNotAllocated("Context"));
    }
    return (T)
        state.instances.updateAndGet(serialID, value -> value != null ? value : provider.provide());
  }

  static IllegalStateException contextNotAllocated(String context) {
    return new IllegalStateException(
        context + " was not allocated using " + Controller.class.getSimpleName());
  }
  ...
  private static final class WorkerScopeController implements Controller {
    ...
    WorkerScopeController(int generators, Thread src, ConcurrentMap<Thread, WorkerState> states) {
    ...
    private WorkerState getOrCreateState(Thread target) {
      if (src == target) {
        srcWorkerState = new WorkerState(new AtomicReferenceArray<>(generators));
        return srcWorkerState;
      }
      WorkerState state = srcWorkerState;
      if (state == null) throw contextNotAllocated("Transfer context");
