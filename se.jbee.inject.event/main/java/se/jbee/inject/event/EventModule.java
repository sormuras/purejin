...
public abstract class EventModule extends BinderModule {
  ...
  public static final class EventBaseModule extends BinderModule {
    ...
    protected void declare() {
      ...
      asDefault()
          .bind(ConnectorBinder.EVENT_CONNECTOR, Connector.class)
      ...
      asDefault()
          .bind(eventTriggerTypeOf(On.Shutdown.class)) //
          .to(EventBaseModule::activateOnShutdown);
    }

    private static void activateOnShutdown(Consumer<On.Shutdown> dispatcher) {
      Runtime.getRuntime().addShutdownHook(new Thread(() -> dispatcher.accept(new On.Shutdown())));
    }
  }
  ...
  // all events use a special dispatcher: ScheduledDispatch which uses a target ID to pick the
  // richtig EventTarget/EventSite/Receiver by this ID (full method name)
