...
public final class SchedulerModule extends BinderModule {
  ...
  /**
   * Main point of this interface is to limit the surface of the API to set in oder to change the
   * execution of the {@link DefaultScheduler}.
   *
   * <p>This does correspond to {@link
  ...
  protected void declare() {
    ...
    asDefault()
        .bind(Name.ANY.in(SCHEDULER_CONNECTOR), raw(Connector.class)) //
    ...
    asDefault()
        .bind(ScheduledExecutor.class) //
        .toProvider(() -> Executors.newSingleThreadScheduledExecutor()::scheduleAtFixedRate);

    asDefault().bind(named(Scheduled.class), ScheduleFactory.class).to(SchedulerModule::annotated);
  ...
  public static Schedule annotated(Object obj, Type<?> as, Method target, Injector context) {
    ...
    if (!property.isEmpty()) {
      intervalMillis =
          context
              .resolve(dependency(Config.class).injectingInto(as))
  ...
  public static class DefaultScheduler implements Consumer<Schedule> {
    ...
    public DefaultScheduler(Injector context, ScheduledExecutor executor) {
      ...
      this.hintsBy = context.resolve(Env.class).in(DefaultScheduler.class).property(HintsBy.class);
    }
    ...
    public void accept(Schedule schedule) {
      ...
      cancellation.set(
          executor.executeInSchedule(
              createTask(schedule, cancellation),
    ...
    private Runnable createTask(Schedule schedule, AtomicReference<Future<?>> cancellation) {
      ...
      Dependency<?> dep =
          dependency(actualReturnType(target, objType)) //
              .injectingInto(objType);
      InjectionSite site =
          new InjectionSite(context, dep, hintsBy.applyTo(context, target, objType));
      Invoke invoke =
          context.resolve(
      ...
      return () -> {
        if (cancellation.get().isCancelled())
          throw new CancellationException("Schedule is cancelled: " + schedule);
        try {
          ...
          if (schedule.cancelAfterFailedRuns()
              && consecutiveFailedRuns.incrementAndGet()
                  >= schedule.cancelAfterConsecutiveFailedRuns) cancellation.get().cancel(true);
  ...
  public static class SchedulerConnector implements Supplier<Connector> {
    ...
    @Override
    public Connector supply(Dependency<? super Connector> dep, Injector context)
    ...
    private void connect(Object instance, Type<?> as, Method connected, String type) {
      ScheduleFactory factory =
          resolverByName.computeIfAbsent(
              type, //
              name -> context.resolve(name, ScheduleFactory.class));
      if (factory == null)
        throw new InconsistentDeclaration("Scheduler factory type not bound: " + type);
