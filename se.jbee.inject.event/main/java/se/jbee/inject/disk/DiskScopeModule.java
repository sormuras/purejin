...
public final class DiskScopeModule extends BinderModule {
  ...
  protected void declare() {
    ...
    asDefault()
        .bind(consumerTypeOf(DiskScope.DiskEntry.class)) //
  ...
  /**
   * By convention {@link Scope#disk(File)} create names starting with {@code disk:} followed by the
   * path to the folder of the scope. This can be used to extract the directory in this {@link
   * Supplier} to bind the particular {@link DiskScope} for that directory.
   */
  private static Scope createDiskScope(Dependency<? super Scope> dep, Injector context) {
    return new DiskScope(
        new File(dep.instance.name.withoutNamespace()),
        TypeDependentScope::instanceSignature,
        context.resolve(consumerTypeOf(DiskScope.DiskEntry.class)));
  }
}
