...
/**
 * Describes a unique action implementation point. That is the particular {@link Method} that
 * implements the {@link Action} for the input parameter {@link Type}.
 *
...
public final class ActionSite<A, B> {
  ...
  public ActionSite(
      ...
      Type<B> out,
      Injector context,
      Consumer<ActionSite<?, ?>> onDisconnect) {
    this.target = target;
    this.in = in;
    this.out = out;
    ...
    Hint<?>[] actualParameters =
        env.property(HintsBy.class).applyTo(context, target.action, target.as, inArg);
    this.injection =
        new InjectionSite(context, dependency(out).injectingInto(target.as), actualParameters);
    this.inputIndex = asList(actualParameters).indexOf(inArg);
  }

  private void disconnect() {
    if (isDisconnected.compareAndSet(false, true)) onDisconnect.run();
  }
  ...
  public Object[] args(Injector context, Object input) {
    ...
    try {
      ...
      throw new ActionExecutionFailed("Failed to provide all implicit arguments", e);
    }
  ...
  public B call(Object[] args, Consumer<Exception> errorHandler) throws ActionExecutionFailed {
    if (isDisconnected()) throw new DisconnectException("Action already disconnected.");
    try {
      Method action = target.action;
      return out.rawType.cast(target.invoke.call(action, target.instance, args));
    } catch (InvocationTargetException ex) {
      if (ex.getTargetException() instanceof DisconnectException) {
        disconnect();
        throw (DisconnectException) ex.getTargetException();
      }
      if (errorHandler != null) errorHandler.accept((Exception) ex.getCause());
      throw new ActionExecutionFailed("Exception on invocation of the action", ex.getCause());
    } catch (Exception ex) {
      if (errorHandler != null) errorHandler.accept(ex);
      throw new ActionExecutionFailed("Exception on invocation of the action", ex);
    }
  }
  ...
  public static final class ActionTarget {
    ...
    public <A, B> boolean isUsableFor(Type<A> in, Type<B> out) {
      if (!returnType().equalTo(out)) return false;
      if (in.equalTo(Type.VOID) && action.getParameterCount() == 0) return true;
      for (Parameter p : action.getParameters()) {
        if (Type.actualParameterType(p, as).equalTo(in)) return true;
      }
      return false;
    }
  }
