...
public interface Action<A, B> {
  ...
  /**
   * Runs the action to compute the output from the input. This typically does not change any inner
   * state but it might in rare cases be part of an action to do such a thing.
   *
   * @param input might be null for {@link Void} arguments or when argument was resolved to null
   * @return might be null
   * @throws ActionExecutionFailed wraps all {@link Exception}s thrown by the underlying method.
   */
