...
/**
 * The {@link MulticastDispatch} calls all {@link ActionSite}s and returns the last successful
 * result.
 */
public final class MulticastDispatch<A, B> implements ActionDispatch<A, B> {
  ...
  public B execute(A input, List<ActionSite<A, B>> sites) {
    ...
    if (sites.size() <= disconnected) throw new DisconnectException("All sites disconnected");
