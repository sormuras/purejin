...
/**
 * When computing something asynchronously that itself returns a {@link Future} the computation
 * becomes a {@link Future} of the original {@link Future}. This implementation wraps this so that
 * type wise this boxing is unboxed again. The computational unboxing will first occur when {@link
 * #get()} or {@link #get(long, TimeUnit)} is called.
 *
 * @since 8.1
...
public final class UnboxingFuture<T> implements Future<T> {
  ...
  public T get() throws InterruptedException, ExecutionException {
    ...
    try {
      ...
      if (e.isCausedByHandlerException()) throw (ExecutionException) e.getCause();
      throw e;
    } catch (Exception e) {
  ...
  public T get(long timeout, TimeUnit unit)
    ...
    try {
      ...
      if (e.isCausedByHandlerException()) throw (ExecutionException) e.getCause();
      if (e.isCausedByTimeout()) throw (TimeoutException) e.getCause();
      throw e;
    } catch (Exception e) {
      throw new ExecutionException(e);
    }
