...
/**
 * Base {@link Module} for modules that want to make known a handler to the event system using
 * {@link #handle(Class)}.
 *
 * @since 8.1
 */
public abstract class ContractModule extends BinderModule {
   ...
   *
   * <p>That means classes implementing the given event interface "automatically" receive calls to
   * any of the interface methods. When The event interface should be injected to signal/call one of
   * its methods a {@link EventProcessor#getProxy(Class)} is injected.
   *
  ...
  public static final class EventBaseModule extends BinderModule {
    ...
    protected void declare() {
      ...
      asDefault().bind(PolicyProvider.class).to(handlerType -> EventPolicy.DEFAULT);
      asDefault()
          .injectingInto(EventProcessor.class)
          .bind(ExecutorService.class)
          .toProvider(Executors::newWorkStealingPool);
    }
  }
}
