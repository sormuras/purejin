...
/**
 * Default implementation of the {@link EventProcessor} supporting all {@link EventPolicy}.
 *
 * @since 8.1
 */
public class ConcurrentEventProcessor implements EventProcessor {
  ...
  private static final class EventHandler<E> {
    ...
    /**
     * If the use succeeds (result is true) the end of usage should be marked by calling {@link
     * #release()} so that this handler can continue to keep track of the concurrent using threads.
     *
     * @return true if this handler could be reserved for usage by the calling thread, else false.
     */
    boolean acquire(Event<?, ?> e) {
      while (true) {
        int calls = concurrentCalls.get();
        if (calls >= e.policy.maxConcurrency) return false;
        if (concurrentCalls.compareAndSet(calls, calls + 1)) return true;
      }
    }
  ...
  static final class EventHandlers<E> extends ConcurrentLinkedDeque<EventHandler<E>> {

    /**
     * Tries to find a handler that can be used to process the event. A successfully received
     * handler has to be marked {@link #release(EventHandler)} right after usage ends unless its
     * handler reference became collected. In that case the handler became out-dated.
     *
  ...
  private <E> EventHandlers<E> getHandlers(Class<E> handlerType, boolean init) {
    return (EventHandlers<E>)
        (init
            ? handlersByType.computeIfAbsent(handlerType, k -> new EventHandlers<>())
  ...
  public <E> void unregister(Class<E> handlerType, E handler) {
    ...
    if (hs != null && !hs.isEmpty()) hs.removeIf(h -> h.handler == handler);
  }
  ...
  public <E> E getProxy(Class<E> handlerType) {
    return (E)
        proxiesByHandlerType.computeIfAbsent(
            handlerType,
            e ->
                newProxyInstance(
                    e.getClassLoader(),
  ...
  // TODO event processing has to be done in a way that makes a try for each target handler - should
  // the event not be completely handled it has to requeue for retries so that other events are
  ...
  public <E> void dispatch(Event<E, ?> event) throws Exception {
    ...
    if (event.policy.isSyncMultiDispatch()) EventException.unwrap(event, res::get);
  }

  // - when should I give up?
  // - what to do when giving up?
  // - how often should I retry?
  // note: these can be programmed as a utility on top with basically the same effect and little
  // extra overhead for a corner case
  //       as long as there is a clear contract: namely that failure is always indicated by a
  // EventException
  @Override
  ...
  private <E, T> T doProcess(Event<E, T> event) throws EventException {
    return event.policy.isAggregatedMultiDispatch() && event.aggregator != null
        ? doDispatch(event)
        : doCompute(event);
  }
  ...
  private <E, T> T doCompute(Event<E, T> event) throws EventException {
    ...
    if (hs == null) throw new EventException(event, null);
    EventHandler<E> h = hs.acquire(event);
    if (h == null)
      throw new EventException(
          event, null); // TODO this should maybe become a Unsupported exception?
    try {
  ...
  private <E, T> T doDispatch(Event<E, T> event) throws EventException {
    ...
    for (EventHandler<E> h : hs) {
      if (h.acquire(event)) {
        ...
        if (needRetry == null) needRetry = new LinkedList<>();
        needRetry.add(h);
      }
    }
  ...
  private static <E, T> T doRetry(Event<E, T> event, LinkedList<EventHandler<E>> needRetry, T res) {
  ...
  private static <T, E> T doAggregate(Event<E, T> event, EventHandler<E> h, T res) {
    final BinaryOperator<T> aggregator = event.aggregator;
    try {
      T res1 = doHandle(event, h.handler);
      return aggregator == null || res == null ? res1 : aggregator.apply(res, res1);
  ...
  private static <E, T> void ensureNotExpired(Event<E, T> event) throws EventException {
    if (event.isExpired()) throw new EventException(event, new TimeoutException());
  }

  @SuppressWarnings("unchecked")
  private static <E, T> T doHandle(Event<E, T> event, E handler) throws EventException {
    try {
  ...
  static class ProxyEventHandler<E> implements InvocationHandler {
    ...
    public ProxyEventHandler(Class<E> handlerType, EventPolicy policy, EventProcessor processor) {
    ...
    @SuppressWarnings("unchecked")
    private <T> Object invoke(Method target, Object[] args, Type<T> result) throws Exception {
      ...
      Event<E, T> e =
          new Event<>(
              ...
              target,
              args,
              (BinaryOperator<T>) defaultAggregator(raw, target.getParameterTypes(), args));
      if (e.returnsVoid()) {
    ...
    private static <T> BinaryOperator<?> defaultAggregator(
        Class<T> rawReturnType, Class<?>[] parameterTypes, Object[] args) {
      BinaryOperator<T> aggregator =
          args != null
                  && args.length > 0
                  && args[args.length - 1] != null
                  && BinaryOperator.class.isAssignableFrom(parameterTypes[args.length - 1])
              ? (BinaryOperator<T>) args[args.length - 1]
              : null;
      if (aggregator != null) return aggregator;
      // TODO otherwise resolve an aggregator function for the type from Injector context
      if (rawReturnType == boolean.class || rawReturnType == Boolean.class) {
