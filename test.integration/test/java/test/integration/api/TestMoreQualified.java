...
class TestMoreQualified {
  ...
  void mostQualifiedIsFirstInSortOrder() {
    HigherNumberIsMoreQualified[] values = new HigherNumberIsMoreQualified[] {hip(1), hip(2)};
  ...
  /**
   * This might sound a bit surprising at first. The reason is that an "unnamed" instance is
   * actually a named instance having the {@link Name#DEFAULT} name. This is so that in case such a
   * instance exists and {@link Name#ANY} is used to resolve it you get the default named instance.
   * Should on the other hand a specific name be resolved it does not matter that the default name
   * is more qualified as it will not match the specific name resolved.
   */
  @Test
  ...
  void specificPackageIsMoreQualifiedThanThatPackageWithItsSubPackages() {
    assertMoreQualified(
        Packages.packageOf(String.class), Packages.packageAndSubPackagesOf(String.class));
  }

  @Test
  void specificPackageIsMoreQualifiedThanSubPackagesUnderIt() {
    assertMoreQualified(Packages.packageOf(String.class), Packages.subPackagesOf(String.class));
  }
  ...
  void explicitSourceIsMoreQualifiedThanPublishedSource() {
    ...
    assertMoreQualified(
        source.typed(DeclarationType.EXPLICIT), source.typed(DeclarationType.PUBLISHED));
  }
  ...
  private static <T extends Qualifying<? super T>> void assertNotMoreQualifiedThanItself(T type) {
