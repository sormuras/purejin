...
/**
 * Check the basic {@link #equals(Object)} and {@link #hashCode()} implementations of value types.
 */
class TestEqualsHashCode {
  ...
  private static void assertValidEqualsAndHashCodeContract(Class<?> cls) throws Exception {
    ...
    assertEquals(
        cls, equals.getDeclaringClass(), cls.getSimpleName() + " does not implement equals");
    Method hashCode = cls.getMethod("hashCode");
    assertEquals(
        cls, hashCode.getDeclaringClass(), cls.getSimpleName() + " does not implement hashCode");
    ...
    for (Field f : cls.getDeclaredFields()) {
      if (!f.isSynthetic()) {
        assertTrue(isFinal(f.getModifiers()), "Field " + f.getName() + " is not final.");
      }
    }
  ...
  private static Object newInstance(Class<?> cls, int base) throws Exception {
    if (cls == int.class || cls == Integer.class) return base;
    if (cls == boolean.class || cls == Boolean.class) return base % 2 == 1;
    if (cls == String.class || cls == Object.class) return String.valueOf(base);
    if (cls == Class.class) return cls;
    if (cls == Name.class) return Name.named(String.valueOf(base));
    if (cls == Type.class) return Type.raw(cls).upperBound(base % 2 == 1);
    if (cls == Instances.class)
      return Instances.ANY.push((Instance<?>) newInstance(Instance.class, base + 1));
    if (cls == Target.class)
      return Target.targeting((Instance<?>) newInstance(Instance.class, base + 1));
    ...
    for (Constructor<?> ci : cls.getDeclaredConstructors()) {
      if (isPublic(ci.getModifiers())
          && (c == null || ci.getParameterCount() > c.getParameterCount())) c = ci;
    }
      ...
      for (Method m : cls.getDeclaredMethods()) {
        if (isPublic(m.getModifiers())
            ...
            && (f == null
                ...
                || m.getParameterCount() == f.getParameterCount() && n == 1)) f = m;
      }
      if (f != null) {
  ...
  private static Object[] createArguments(Class<?> cls, int base, Class<?>[] types)
      throws Exception {
