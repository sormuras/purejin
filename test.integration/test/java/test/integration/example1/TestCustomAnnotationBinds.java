...
/**
 * This example shows that custom annotations can be provided via {@link ServiceLoader} using {@link
 * ModuleWith} as contract. This has the benefit of plugging into the bootstrapping without
 * additional setup code but has the drawback that the class implementing the annotation effect
...
class TestCustomAnnotationBinds {
  ...
  /**
   * This verifies that the {@link Support} annotation's effect is loaded via {@link ServiceLoader}.
   * It is defined in the com.example.app test dependency jar file. If the {@link MySupportService}
   * can be resolved it was bound which meant the annotation had done its effect. Otherwise no
