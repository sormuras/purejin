...
/**
 * Tests the bootstrapping process using the {@link DefaultEnv} as the first {@link Env} so that
 * further {@link Env} and {@link se.jbee.inject.Injector}s can be bootstrapped.
 */
class TestDefaultEnv {
  ...
  private static void assertDefined(Env env, Type<?> property) {
    assertDoesNotThrow(() -> assertNotNull(env.property(property)));
  }
  ...
  private static void assertDefined(Env env, String qualifier, Type<?> property) {
    assertDoesNotThrow(() -> assertNotNull(env.property(qualifier, property)));
  }
}
