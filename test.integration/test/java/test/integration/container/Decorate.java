...
public class Decorate {
   ...
   * @return An {@link Injector} with both contexts where root acts as fallback
   */
  public static Injector hierarchy(Injector root, Injector branch) {
    return new Injector() {
      ...
      public <T> T resolve(Dependency<T> dep) throws UnresolvableDependency {
        if (dep.type().arrayDimensions() == 1) return Decorate.resolveArray(dep, root, branch);
        try {
  ...
  static <T> T resolveArray(Dependency<T> dep, Injector root, Injector branch) {
    ...
    Object arr = Array.newInstance(dep.type().baseType().rawType, rootLength + branchLength);
