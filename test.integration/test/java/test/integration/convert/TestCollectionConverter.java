...
class TestCollectionConverter {
  ...
  public static class TestCollectionConverterModule extends ConverterModule {
    ...
    public static <B> Converter<String, B[]> toArray(
        Type<B> elementType, Converter<String, B> elementConverter) {
      return in -> arrayMap(in.split(","), elementType.rawType, elementConverter::convert);
    }
  }

  private final Injector context = Bootstrap.injector(TestCollectionConverterModule.class);

  @Test
  void genericArrayToCollectionConverterCanBeDefined() {
    Converter<Number[], List<Number>> arr2list =
        context.resolve(Converter.converterTypeOf(raw(Number[].class), listTypeOf(Number.class)));
    assertEquals(asList(1, 2), arr2list.convert(new Number[] {1, 2}));
  }

  @Test
  void genericStringToArrayConverterCanBeDefined() {
    Converter<String, Integer[]> str2intArr =
        context.resolve(Converter.converterTypeOf(String.class, Integer[].class));
    assertArrayEquals(new Integer[] {42, 13}, str2intArr.convert("42,13"));
  }
}
