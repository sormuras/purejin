...
class TestConverter {
  ...
  public static class TestConverterModule extends ConverterModule {
    ...
    public <B> Converter<String, B[]> toArray(Type<B> elementType, Converter<String, B> element) {
      return input -> arrayMap(input.split("\\s*,\\s*"), elementType.rawType, element::convert);
    }
  ...
  void chainsCanBeBuildByAppending() {
    ...
    assertEquals(Integer.valueOf(13), str2long.then(long2int).convert("13"));
  }
  ...
  void chainsCanBeBuildByPrepending() {
    Converter<String, Long> str2long = Long::parseLong;
    Converter<Long, Integer> long2int = Long::intValue;
    assertEquals(Integer.valueOf(13), long2int.upon(str2long).convert("13"));
  }
  ...
  private final Injector context = Bootstrap.injector(TestConverterModule.class);

  @Test
  void converterMethodsWithTypeVariableUseScopeDependencyType() {
    @SuppressWarnings("rawtypes")
    Resource<Converter<String, List>> str2ints =
        context.resolve(
            resourceTypeOf(Converter.converterTypeOf(raw(String.class), classType(List.class))));
    assertEquals(Scope.dependencyType, str2ints.lifeCycle.scope);
  }

  @Test
  void genericMethodConverters() {
    Converter<String, List<Integer>> str2ints =
        context.resolve(Converter.converterTypeOf(raw(String.class), listTypeOf(Integer.class)));
    assertEquals(asList(42, 13), str2ints.convert("42, 13"));
    Converter<String, List<Long>> str2longs =
        context.resolve(Converter.converterTypeOf(raw(String.class), listTypeOf(Long.class)));
