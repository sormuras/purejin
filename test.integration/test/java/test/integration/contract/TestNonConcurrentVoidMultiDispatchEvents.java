...
/**
 * This test illustrates the most basic scenario of a classic "event listener" case where
 * calls/messages are dispatched to multiple listeners for different events (here {@link
 * Listener#onX(int)} and {@link Listener#onY(int)}).
 *
 * <p>While the dispatch is asynchronous the test setting uses a {@link EventPolicy#maxConcurrency}
 * of 1 which only allows one thread calling any of the two methods at the same time which means the
 * other message can only be delivered after the first has been delivered successful.
 *
 * <p>Indirectly this test also makes sure the multi-dispatch for void returning methods is
 * asynchronously processed as the event processors worker threads both will be waiting in the
 * {@link Service} methods to allow to check state which will only resolve if the main thread
 * continues in the test method to the point where it notifies the waiting worker threads.
 */
class TestNonConcurrentVoidMultiDispatchEvents {
  ...
  private static final class TestMultiDispatchEventsModule extends ContractModule {
    ...
    protected void declare() {
      ...
      bind(PolicyProvider.class).to(event -> EventPolicy.DEFAULT.withMaxConcurrency(1));
    }
  }
  ...
  void onlyOneThreadAtATimeCallsSameListersMethods() {
    ...
    listener.onY(13);
    giveSomeTime();
    // there are two scenarios as we cannot be sure which message is first processed to the point
    // where the onX/onY is called
    if (a.xs.isEmpty()) {
      assertMessages(a.xs);
    ...
    giveSomeTime();
    // no change as b is still in the handler method and thereby blocks the other event from sending
    // it to b
    if (a.xs.isEmpty()) {
      assertMessages(a.xs);
      assertMessages(a.ys, 13);
  ...
  /**
   * Called when the {@link EventProcessor} should be given some time to run the expected events.
   */
  ...
  private static void assertMessages(List<Integer> actual, int... expected) {
    assertEquals(expected.length, actual.size(), "Number of message not same:");
    for (int i = 0; i < expected.length; i++) assertEquals(expected[i], actual.get(i).intValue());
  }
}
