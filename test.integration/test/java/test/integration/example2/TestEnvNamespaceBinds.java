...
/**
 * This test example shows how {@link se.jbee.inject.binder.LocalEnvModule} can be used to set
 * strategies like {@link se.jbee.inject.config.ProducesBy} and {@link
 * se.jbee.inject.config.NamesBy} for a {@link Package} and their subpackages.
 *
 * <p>In this example there is a {@link Car} and a {@link Bus} related package, one picking up
 * {@link Car}s when using {@link Binder.ScopedBinder#autobind()}, the other picking up {@link
 * Bus}ses. Within the {@link Car} package is a subpackages with a {@link Car[]} pool override. Last
 * but not least this pool behaviour is still set in the sub-package of the corporate {@link Car[]}
 * pool.
 *
 * <p>On the concept level this example verifies the general principle of local name-spaces in the
 * {@link se.jbee.inject.Env} and that the base class {@link se.jbee.inject.binder.LocalEnvModule}
 * affects the made bindings accordingly.
 *
 * <p>The reason this example only uses {@link Binder.ScopedBinder#autobind()} is that is also
 * should test the {@link se.jbee.inject.config.ProducesBy} and {@link
 * se.jbee.inject.config.NamesBy} strategies are resolved in the localised name-space.
 */
