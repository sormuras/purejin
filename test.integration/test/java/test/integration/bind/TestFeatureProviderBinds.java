...
/**
 * {@link Provider}s are a common concept in dependency injection used to inject instances with an
 * unstable {@link Scope} into those with a more stable {@link Scope} without running the risk of
 * accessing stale state.
 *
 * <p>This is simply done by not injecting the unstable type directly into the more stable one but
 * by wrapping it into the {@link Provider} indirection. To access the current valid value when
 * needed {@link Provider#provide()} is called right before the value is needed.
 *
 * <p>In most dependency injection frameworks and libraries the burden to use {@link Provider}s
 * where needed lies upon the user and mistakes simply manifest inconsistent runtime behaviour and
 * the underlying scoping issue has to be identified by the user.
 *
 * <p>To lift this burden purejin has the concept of {@link ScopeLifeCycle}s where {@link Scope}s
 * declare their stability or consistency in relation to each other. This is done as part of the
 * {@link se.jbee.inject.defaults.DefaultScopes} but users can define their own scopes or redefine
 * existing ones with different {@link ScopeLifeCycle}. The {@link Injector} uses the {@link
 * ScopeLifeCycle} information to detect faulty relations at injection.
 *
 * <p>The {@link Provider} can then be used to overcome the limitations and allow injecting unstable
 * dependencies into stable ones. By default this feature is not installed. To enable it install
 * {@link DefaultFeature#PROVIDER}.
 */
class TestFeatureProviderBinds {
  ...
  static final Instance<WorkingStateConsumer> A =
      instance(named("A"), raw(WorkingStateConsumer.class));
  static final Instance<WorkingStateConsumer> B =
      instance(named("B"), raw(WorkingStateConsumer.class));
  ...
  public static class WorkingStateConsumer {
    ...
    public WorkingStateConsumer(Provider<DynamicState> state, Provider<String[]> strings) {
      this.state = state;
  ...
  private final Injector context = Bootstrap.injector(TestFeatureProviderBindsModule.class);
  ...
  void providersAreAvailableForArrays() {
    WorkingStateConsumer state = context.resolve(WorkingStateConsumer.class);
  ...
  void providersAreAvailableForLists() {
    ...
    assertInjectsProviderFor(list, raw(List.class).parameterized(String.class));
  }
  ...
  void scopingConflictsCauseException() {
    Exception ex =
        assertThrows(UnstableDependency.class, () -> context.resolve(FaultyStateConsumer.class));
    assertEquals(
        "Unstable dependency injection\n"
            + "\t  of: test.integration.bind.TestFeatureProviderBinds.DynamicState scoped"
            + " injection\n"
            + "\tinto: test.integration.bind.TestFeatureProviderBinds.FaultyStateConsumer scoped"
            + " application",
        ex.getMessage());
  }
  ...
  void providersCanBeCombinedWithOtherBridges() {
    Provider<List<WorkingStateConsumer>> provider =
        context.resolve(providerTypeOf(listTypeOf(WorkingStateConsumer.class)));
  ...
  private <T> void assertInjectsProviderFor(
      T expected, Type<? extends T> dependencyType, Name name) {
    Provider<?> provider = context.resolve(name, providerTypeOf(dependencyType));
