...
class TestBasicParentLocalBinds {
  ...
  private static class TestBasicParentLocalBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      injectingInto(Child.class)
          .within(Parent.class) //
          .bind(String.class)
          .to("child-with-parent");
      construct(Grandparent.class);
      injectingInto(Child.class)
          .within(Parent.class)
          .within(Grandparent.class) //
          .bind(String.class)
          .to("child-with-grandparent");
    }
  }
  ...
  private final Injector context = Bootstrap.injector(TestBasicParentLocalBindsModule.class);

  @Test
  void bindWithParentTargetIsUsedWhenInjectionHasParent() {
    assertEquals("child-with-grandparent", context.resolve(Grandparent.class).parent.child.value);
    assertEquals("child-with-parent", context.resolve(Parent.class).child.value);
