...
/**
 * Plug-in-binds are a convenient way to define a named set of classes. The name of the set is given
 * by the class stated in the {@link PluginBinder#into(Class)} method. The {@link
 * Binder#plug(Class)} states the class that should be added to a set.
 *
 * <p>The set than can be received from the {@link Injector} using {@link Plugins#forPoint(Class)}.
 * This allows to easily build abstractions that use this to collect the set of classes their effect
 * applies to.
 *
 * <p>As the name suggest this is a perfect way to build a plugin system.
 *
 * <p>Note that the sets are just ordinary bindings of type {@link Class}. That means the sets can
 * contain different members depending on the actual {@link Dependency} if the scope of the bind is
 * narrowed (localised) like it is done in this example.
 */
class TestBasicPluginBinds {
  ...
  private static class TestBasicPluginBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      inPackageOf(Module.class).plug(ExtensionPackageLocalAction.class).into(Callable.class);
      injectingInto(Serializable.class).plug(ExtensionInstanceOfAction.class).into(Callable.class);
      ...
      injectingInto(ContextAwarePlugins.class).plug(String.class).into(CharSequence.class);
    }
  }
  ...
  private final Injector context = Bootstrap.injector(TestBasicPluginBindsModule.class);
  ...
  void packageLocalExtensionsAreResolvedWithAppropriateInjection() {
    Class<?>[] classes = plugins.targeting(Module.class).forPoint(Callable.class);
    assertEqualsIgnoreOrder(
        new Class<?>[] {ExtensionAction.class, ExtensionPackageLocalAction.class}, classes);
  }

  @Test
  void instanceOfExtensionsAreResolvedWithAppropriateInjection() {
    Class<?>[] classes = plugins.targeting(String.class).forPoint(Callable.class);
    assertEqualsIgnoreOrder(
        new Class<?>[] {ExtensionAction.class, ExtensionInstanceOfAction.class}, classes);
  }
  ...
  void pluginsAreInjectedContextAware() {
    ...
    assertEqualsIgnoreOrder(
        new Class<?>[] {String.class}, bean.plugins.forPoint(CharSequence.class));
  }
}
