...
/** How to inject different arrays into different instances of the same parent type. */
class TestBasicArrayBinds {
  ...
  static class TestBasicArrayBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      injectingInto("command1", Command.class).multibind(Number.class).to(PRE_2, Double.class);

      bind("command2", Command.class).toConstructor();
      injectingInto("command2", Command.class).multibind(Number.class).to(PRE_2, Double.class);
      ...
      bind("command4", Command.class).toConstructor(Hint.constant(new Number[] {2d, 9}));

      bind("command5", Command.class).toConstructor();
      injectingInto("command5", Command.class).arraybind(Number[].class).toElements(1, 2, 3);

      bind("command6", Command.class).toConstructor();
      injectingInto("command6", Command.class).bind(Number[].class).to(new Number[] {4, 5, 6});
    }
  }

  private final Injector injector = Bootstrap.injector(TestBasicArrayBindsModule.class);
  ...
  private void assertPreconditions(String command, Number... expected) {
    assertEqualsIgnoreOrder(expected, injector.resolve(command, Command.class).preconds);
  }
}
