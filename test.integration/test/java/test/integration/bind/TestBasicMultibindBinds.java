...
/**
 * A {@link se.jbee.inject.binder.Binder.TypedBinder#multibind(Type)} is a normal bind except that
 * it declares that further binds for the same {@link Name} and {@link Type}, in other words for the
 * same {@link se.jbee.inject.Instance} are valid. In such cases the receiver usually expects a
 * collection of matching dependencies and all that have been bound with {@link
 * se.jbee.inject.DeclarationType#MULTI} are injected.
 *
 * <p>If such binds would instead use a normal {@link
 * se.jbee.inject.binder.Binder.TypedBinder#bind(Type)}, which makes them a {@link
 * se.jbee.inject.DeclarationType#EXPLICIT} bind, bindings of the same {@link Name} and {@link Type}
 * do clash with each other and cause an exception during bootstrapping of the {@link Injector}
 * context.
 */
class TestBasicMultibindBinds {

  /**
   * Just split to show that usually multiple {@code multibind}s are made in different {@link
  ...
  private final Injector context = Bootstrap.injector(TestBasicMultibindBindsBundle.class);
