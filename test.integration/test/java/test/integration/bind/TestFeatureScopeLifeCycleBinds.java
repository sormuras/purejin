 ...
 *
 * <p>BTW both {@link Scope}s and {@link ScopeLifeCycle}s are managed instances that can be resolved
 * in the {@link Injector} context.
 *
 * <p>{@link Scope} implementation may also have dependencies.
 */
class TestFeatureScopeLifeCycleBinds {
  ...
  static final class TestFeatureScopeLifeCycleBindsModule extends BinderModule {
    ...
    protected void declare() {
      bindLifeCycle(requestScope)
          .toFactory(
              context -> context.resolve(Scope.worker, ScopeLifeCycle.class).derive(requestScope));
    }
  }

  private final Injector context = Bootstrap.injector(TestFeatureScopeLifeCycleBindsModule.class);
  ...
  void defaultScopeLifeCyclesAreBoundAsDefaults() {
    for (Resource<ScopeLifeCycle> r :
        context.resolve(Resource.resourcesTypeOf(ScopeLifeCycle.class))) {
  ...
  void defaultScopesAreBoundAsDefaults() {
    for (Resource<Scope> r : context.resolve(Resource.resourcesTypeOf(Scope.class))) {
      if (!r.signature.instance.name.equalTo(requestScope))
        assertEquals(DeclarationType.DEFAULT, r.source.declarationType);
    }
  }
