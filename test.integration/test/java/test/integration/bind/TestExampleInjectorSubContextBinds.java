...
/**
 * A test that demonstrates how {@link Bundle}s (here in form of {@link BinderModule}s) are
 * installed for lazy bootstrapped {@link Injector} sub-contexts.
 *
 * <p>This form of {@link Injector} nesting is not limited to any depth. In this example the first
 * level is the "foo" sub-context, the second level is the "bar" sub-context defined in "foo".
 */
class TestExampleInjectorSubContextBinds {
  ...
  private final Injector injector =
      Bootstrap.injector(TestExampleInjectorSubContextBindsModule.class);

  @Test
  void bindsInstalledInSubContextAreNotAccessibleInParent() {
    assertThrows(UnresolvableDependency.class, () -> injector.resolve(int.class));
  }
  ...
  void subContextEnvironmentIsSameAsRootContexts() {
    Injector foo = injector.subContext("foo");
    assertNotSame(foo, injector);
    assertSame(injector.resolve(DEFAULT, Env.class), foo.resolve(DEFAULT, Env.class));
  }
}
