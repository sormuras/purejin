 ...
 *
 * <p>This test demonstrates how the {@link DefaultValueBinders#REFERENCE_PREFER_CONSTANTS} can be
 * used to avoid bindings of type {@link BindingType#REFERENCE} where possible.
 *
 * <p>This is the case when the referenced {@link Class} is banal in its construction and it has no
 * instance state on which it could depend. In such a case the {@link
 * DefaultValueBinders#REFERENCE_PREFER_CONSTANTS} will not create a reference to the referenced
 * type and a implicit bind to constructor but a direct bind to a constant instance created during
 * bootstrapping.
 *
 * <p>This might make sense in some situation but cause issues in others when there cannot be
 * multiple instances of the same type anyway for some reason.
 *
...
class TestExamplePreferConstantsBinds {
  ...
  private static Injector injectorWithEnv(
      Class<? extends Bundle> root, ValueBinder<Instance<?>> binder) {
    return Bootstrap.injector(
        Bootstrap.DEFAULT_ENV.with(valueBinderTypeOf(Instance.class), binder), root);
  }
}
