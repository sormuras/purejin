...
/**
 * The test demonstrates the {@link DefaultFeature#PRIMITIVE_ARRAYS} that installs a bridge between
 * wrapper arrays and their primitive counterparts so that values of wrappers (and their
 * 1-dimensional array types) can be resolved as primitive arrays as well.
 *
...
class TestBasicPrimitiveArrayBridgeBinds {
  ...
  private static class TestBasicCustomPrimitiveArrayBindsModule extends BinderModule {
  ...
  private final Injector context =
      Bootstrap.injector(TestBasicCustomPrimitiveArrayBindsModule.class);
  ...
  void primitiveArrayBridgeProvidesPrimitiveFloatArray() {
    ...
    for (float v : floats) actual.add(v);
  ...
  void primitiveArrayBridgeProvidesPrimitiveDoubleArray() {
    ...
    for (double v : doubles) actual.add(v);
