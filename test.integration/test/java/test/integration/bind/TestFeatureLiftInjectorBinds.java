...
/**
 * A test that demonstrates that the {@link Injector} can be decorated using a {@link Lift} and that
 * the decorated {@link Injector} is passed to {@link Supplier}s when resolving {@link Dependency}s.
 */
class TestFeatureLiftInjectorBinds {

  public static class DecoratingInjector implements Injector, Lift<Injector> {
    ...
    @Override
    public <T> T resolve(Dependency<T> dependency) throws UnresolvableDependency {
      return decorated.resolve(dependency);
    }

    @Override
    public Injector lift(Injector target, Type<?> as, Injector context) {
      // OBS: context is the undecorated Injector (there might have been other decorations applied
      // before)
      // use target instead!
  ...
  static class TestFeatureLiftInjectorBindsModule extends BinderModule {
    ...
    Bean supply(@SuppressWarnings("unused") Dependency<? super Bean> dep, Injector context)
  ...
  private static Injector injector = Bootstrap.injector(TestFeatureLiftInjectorBindsModule.class);
