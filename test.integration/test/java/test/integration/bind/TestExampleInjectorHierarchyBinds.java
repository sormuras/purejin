...
/**
 * Test that demonstrates how two {@link Injector} contexts can be linked together in a hierarchy
 * using the {@link Decorate} utility.
 */
class TestExampleInjectorHierarchyBinds {
  ...
  void childContextIsAccessibleForHierarchicalInjector() {
    assertEquals(42.42f, branched1.resolve(Float.class), 0.01f);
    assertEquals(0.42f, branched2.resolve(Float.class), 0.01f);
  }

  @Test
  void mergedArrayContextIsAccessibleForHierarchicalInjector() {
    Number[] numbers = branched1.resolve(Type.raw(Number[].class).asUpperBound());
