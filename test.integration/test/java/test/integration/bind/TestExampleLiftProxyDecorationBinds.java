...
/**
 * Tests demonstrates how to use {@link Lift}s to decorate the {@link Injector} or bound instances.
 */
class TestExampleLiftProxyDecorationBinds {
  ...
  private static class TestExampleLiftProxyDecorationBindsModule extends BinderModule
      implements Lift<Shape> {
    ...
    protected void declare() {
      ...
      bind(Shape.class)
          .toProvider(
              () ->
                  new Area()); // OBS: A constant would not work as it is resolved using a shortcut
  ...
  private final Injector injector =
      Bootstrap.injector(TestExampleLiftProxyDecorationBindsModule.class);
  ...
  void injectedInjectorCanBeDecoratedUsingLifts() {
    assertSame(DelegatingInjector.class, injector.resolve(Injector.class).getClass());
    assertSame(DelegatingInjector.class, injector.resolve(Foo.class).injector.getClass());
  }
