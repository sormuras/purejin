 ...
 *
 * <p>As the name 'auto' suggest this is a way of binding {@link java.lang.reflect.Constructor}s,
 * {@link java.lang.reflect.Method}s and {@link java.lang.reflect.Field} in a (semi-) automatic way.
 *
 * <p>The user configures the selection process using strategy interfaces {@link
 * se.jbee.inject.config.ConstructsBy}, {@link ProducesBy}, {@link AccessesBy} to programmatically
 * describe how to find and select the members to use and bind. The created bindings can be further
 * customised using the {@link se.jbee.inject.config.ScopesBy} strategy to decide the {@link
 * se.jbee.inject.Scope} used, and {@link se.jbee.inject.config.HintsBy} to decides what arguments
 * to use for {@link java.lang.reflect.Parameter}s of bound {@link java.lang.reflect.Executable}s.
 *
 * <p>All this strategies are "globally" bound in the {@link se.jbee.inject.Env} simply using {@link
 * Env#with(Class, Object)}. These settings can be adjusted, overridden or replaced per {@link
 * BinderModule} by overriding it {@link BinderModule#configure(Env)} method. Last but not least the
 * strategies can be set individually just for the {@link Binder.ScopedBinder#autobind()} call using
 * its {@link se.jbee.inject.binder.Binder.AutoBinder#produceBy(ProducesBy)} (and similar) methods.
 */
class TestBasicAutoBinds {
  ...
  public static class TestBasicAutoBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      autobind()
          .nameBy(NamesBy.DECLARED_NAME) // use method names to avoid clashing binds
          .produceBy(
              flat.returnTypeAssignableTo(raw(boolean.class)) // returning booleans
                  ...
                  .or(
                      flat.returnTypeAssignableTo(
                          raw(int.class)))) // as well as any method returning an int
  ...
  private final Injector context = Bootstrap.injector(TestBasicAutoBindsModule.class);
  ...
  void methodSelectionWithOr() {
    ...
    assertEqualsIgnoreOrder(new Integer[] {2, 3, 12}, context.resolve(Integer[].class));
    assertThrows(
        UnresolvableDependency.class, () -> context.resolve(double.class), "should not be bound");
    assertThrows(
        UnresolvableDependency.class, () -> context.resolve(float.class), "should not be bound");
  }
