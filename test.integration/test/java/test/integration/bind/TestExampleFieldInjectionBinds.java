 ...
 *
 * <p>By default the {@link Injector} expects constructor injection. It does neither field nor
 * setter injection as these lead to problematic code patterns. This example shows how field
 * injection could be added anyhow.
 *
 * <p>The {@link FieldInjectionBinder} shows how one could inject fields when an instance is created
 * by using a custom {@link ValueBinder} that decorates the original {@link Supplier} to also inject
 * fields.
 *
 * <p>It should be noted that this is just one possible way to add a feature like field injection.
 * Another solution would be the use of {@link Lift}s.
 *
...
class TestExampleFieldInjectionBinds {
  ...
  private static class TestExampleFieldInjectionBindsModule extends BinderModule {
  ...
  /**
   * A simple example-wise {@link Supplier} that allows to initialised newly created instances.
   *
   * <p>In this example a very basic field injection is build but it could be any kind of context
   * dependent instance initialisation.
   */
  ...
  static final class FieldInjectionBinder implements ValueBinder<Constructs<?>> {
    ...
    @Override
    public <T> void expand(Env env, Constructs<?> ref, Binding<T> item, Bindings dest) {
      Supplier<T> supplier =
          new FieldInjectionSupplier<>(Supply.byConstruction(ref.typed(item.type())));
      dest.addExpanded(env, item.complete(CONSTRUCTOR, supplier));
    }
  }
  ...
  void fieldInjectionTookPlace() {
    Injector injector =
        injectorWithEnv(TestExampleFieldInjectionBindsModule.class, new FieldInjectionBinder());
    assertEquals("answer", injector.resolve(Bean.class).s);
  }

  private static Injector injectorWithEnv(
      Class<? extends Bundle> root, ValueBinder<Constructs<?>> binder) {
    return Bootstrap.injector(
        Bootstrap.DEFAULT_ENV.with(valueBinderTypeOf(Constructs.class), binder), root);
  }
}
