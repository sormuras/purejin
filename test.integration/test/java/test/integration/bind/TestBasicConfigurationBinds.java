...
/**
 * The {@link Config} {@link se.jbee.inject.config.Extension} is a helper to organise runtime
 * configuration of the application that is made as part of the binding process.
 *
 * <p>Localised binds are used to declare the individual configuration properties. To ease this
 * process the {@link se.jbee.inject.binder.Binder.ScopedBinder#configure(Class)} helps to
 * understand localisation as configuration for a specific target {@link Class} or {@link
 * se.jbee.inject.Instance}.
 *
 * <p>This configuration is then accessed by injecting the {@link Config} into the target type.
 */
class TestBasicConfigurationBinds {
  ...
  private static final class TestBasicConfigurationBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      bind(converterTypeOf(String.class, UUID.class)).to(UUID::fromString);
      configure().bind(named("uuid"), String.class).to(UUID.randomUUID().toString());
    }
  }

  private final Injector injector = Bootstrap.injector(TestBasicConfigurationBindsModule.class);
  ...
  void unknownValueThrowsException() {
    Optional<String> unknown = config.optionalValue(String.class, "unknown");
