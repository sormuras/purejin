 ...
 *
 * <p>The example also shows how {@link Annotation}s like {@link Featured} can be used to mark
 * {@link se.jbee.inject.bind.Bundle}s or {@link Module}s as a specific {@link Feature}.
 */
class TestFeatureEditionFeatureBinds {
  ...
  private enum FeaturedOptionBundle implements Dependent<FeaturedOptionBundle> {
    QUX;

    @Override
    public void bootstrap(Bootstrapper.DependentBootstrapper<FeaturedOptionBundle> bootstrapper) {
  ...
  void theFeaturedBundlesAndModulesAreInstalled() {
    assertEditionInstalls(Edition.includes(AnnotatedFeature.BAR, AnnotatedFeature.BAZ), 8, 128);
  }
