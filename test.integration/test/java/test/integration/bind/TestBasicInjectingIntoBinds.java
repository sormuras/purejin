...
/**
 * A test that shows how to inject a specific instance into another type using the {@link
 * ScopedBinder#injectingInto(se.jbee.inject.Instance)} API.
 *
 * <p>In other words, how to create bindings that have an effect that is local to the target
 * instance identified by {@link Name} and {@link Type}.
 *
 * <p>This sort of narrowing the validity of bindings is also called targeting. The information
 * build using the fluent API ends up in a {@link se.jbee.inject.Resource}'s {@link
 * se.jbee.inject.Target}. The {@link se.jbee.inject.Target} describes the scenario in which the
 * {@link se.jbee.inject.Resource} is valid to use.
 *
 * <p>Besides the receiving target instance the {@link se.jbee.inject.Target} can be narrowed to the
 * parent and grandparent instances of the receiving instance up the injection hierarchy as well as
 * to the {@link se.jbee.inject.Packages} in which the target instance is declared.
 *
 * <p>Last but not least the {@link se.jbee.inject.Target} can limit access to only allow indirect
...
class TestBasicInjectingIntoBinds {
  ...
  /**
   * We use different {@link B} constants to check if the different {@link A}s got their desired
   * {@linkplain B}.
   */
  ...
  private static class TestBasicInjectingIntoBindsModule extends BinderModule {
  ...
  private final Injector context = Bootstrap.injector(TestBasicInjectingIntoBindsModule.class);
  ...
  @Test
  void
      bindWithExactClassTargetIsUsedWhenInjectingIntoClassHavingThatClassButAlsoAnInterfaceMatching() {
