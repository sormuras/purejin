...
class TestBasicGeneratorBinds {
  ...
  private final Injector context = Bootstrap.injector(TestBasicGeneratorBindsModule.class);
  ...
  void toGeneratorBypassesScopeEffects() {
    ...
    assertEquals(
        "NonScopedGenerator",
        context.resolve(resourceTypeOf(String.class)).generator.getClass().getSimpleName());
  }
  ...
  void toScopedGeneratorDoesNotBypassScopeEffects() {
    ...
    assertEquals(
        ...
        "second time is the real test - if this is scoped this is a application singleton so the"
            + " value is still 1");
    // another way to verify it...
    assertEquals(
        "LazyScopedGenerator",
        context.resolve(resourceTypeOf(int.class)).generator.getClass().getSimpleName());
  }
}
