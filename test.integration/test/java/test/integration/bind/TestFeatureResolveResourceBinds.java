 ...
 * se.jbee.inject.bind.Module}s creates {@link se.jbee.inject.bind.Binding}s which become {@link
 * ResourceDescriptor} which become {@link Resource}s within the {@link Injector} context.
 *
 * <p>Each {@link Resource} is a {@link Generator} plus its description originating from the {@link
 * se.jbee.inject.bind.Binding} that created it. {@link Resource}s can be resolved themselves as if
 * they had been bound as a consequence of a bound type.
 *
 * <p>So when binding a {@link String} one can resolve {@link Resource#resourceTypeOf(Type)} of
 * {@link String} to access the {@link Resource} that is responsible for generating the actual
 * instance.
 *
 * <p>This feature can be used to "re-resolve" the factories for the arguments of {@link
 * java.lang.reflect.Constructor}s and {@link java.lang.reflect.Method}s reducing the re-occurring
 * work needed to inject these in case these are bound in a {@link Scope} where they are called
 * often.
 *
 * <p>This test verifies that both {@link Resource} and {@link Generator}s can be resolved for every
 * bound type.
 */
class TestFeatureResolveResourceBinds {

  private static class TestFeatureResolveResourceBindsModule extends BinderModule {
  ...
  private final Injector context = Bootstrap.injector(TestFeatureResolveResourceBindsModule.class);

  @Test
  void resourceIsAvailableForEveryBoundResource() {
    Resource<String> resource = context.resolve(Resource.resourceTypeOf(String.class));
  ...
  void resourceArrayIsAvailableForEveryBoundResource() {
    Resource<String>[] resource = context.resolve(Resource.resourcesTypeOf(String.class));
  ...
  void resourceArrayFiltersByName() {
    Dependency<? extends Resource<String>[]> dependency =
        dependency(Resource.resourcesTypeOf(String.class)).named("special");
  ...
  void generatorIsAvailableForEveryBoundResource() {
    Generator<String> generator = context.resolve(Generator.generatorTypeOf(raw(String.class)));
  ...
  void generatorArrayIsAvailableForEveryBoundResource() {
    Generator<String>[] gens = context.resolve(Generator.generatorsTypeOf(raw(String.class)));
