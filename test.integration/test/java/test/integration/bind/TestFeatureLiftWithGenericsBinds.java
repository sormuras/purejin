...
class TestFeatureLiftWithGenericsBinds {
  ...
  static class TestFeatureLiftWithGenericsBindsModule extends BinderModule {
    ...
    protected void declare() {
      lift(listTypeOf(String.class))
          .to(
              (target, as, context) -> {
                target.add("a");
                return target;
              });
      lift(listTypeOf(Integer.class))
          .to(
              (target, as, context) -> {
                target.add(1);
                return target;
              });
      lift(StringList.class)
          .to(
              (target, as, context) -> {
                target.add("b");
                return target;
  ...
  private final Injector context = Bootstrap.injector(TestFeatureLiftWithGenericsBindsModule.class);

  @Test
  void liftOnlyAffectsActualTypesThatAreAssignable() {
    assertEquals(
        new HashSet<>(asList("b", "a")), new HashSet<>(context.resolve(listTypeOf(String.class))));
