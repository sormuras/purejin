...
class TestFeatureObtainableBinds {
  ...
  private final Injector context = Bootstrap.injector(TestObtainableBindsModule.class);

  @Test
  void obtainsArrayElementsThatCouldBeResolved() {
    Manager manager = context.resolve(Manager.class);
    assertEquals(
        asList("a", "b"), Arrays.stream(manager.plugins).map(p -> p.name).collect(toList()));
  }
  ...
  void obtainsEmptyArray() {
    assertArrayEquals(
        ...
        context
            .resolve( //
                obtainableTypeOf(Integer[].class))
            .obtain());
  }
  ...
  void obtainsOriginalExceptionWhenInstanceCanNotBeResolved() {
    ...
    Exception ex = assertThrows(UnresolvableDependency.class, box::orElseThrow);
    assertEquals(
