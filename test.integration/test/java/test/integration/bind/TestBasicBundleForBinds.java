...
/**
 * The test demonstrates how to use {@link Dependent} and the {@link BundleFor} to allow different
 * bootstrapping depended on a toggle property set in the {@link Env}.
 *
 * <p>This technique avoids using if-statements in the {@link Bundle}s and {@link Module}s itself to
 * get manageable and predictable sets of configurations that can be composed easily using arguments
 * to the bootstrapping process itself.
 *
 * <p>In this example we use {@link Binder#multibind(Class)}s to show that just one of the bundles
 * has been installed depending on the value we defined {@link Env} for the {@link Machine} which is
 * our toggle property.
 *
...
class TestBasicBundleForBinds {
  ...
  private static class TestBasicBundleForBindsBundle extends BootstrapperBundle {
  ...
  /**
   * The {@link GenericMachineBundle} will be used when no {@link Machine} value has been defined in
   * the {@link Env} so that it is actually <code>null</code>.
   */
