...
/**
 * A test that demonstrates how to make binds that only affect classes in specific packages and/or
 * sub-packages using {@link TargetedBinder#in(se.jbee.inject.Packages)} or any of the utility
 * methods on top of it: {@link TargetedBinder#inPackageOf(Class)}, {@link
 * TargetedBinder#inSubPackagesOf(Class)} and {@link
 * TargetedBinder#inPackageAndSubPackagesOf(Class)}.
 *
 * <p>This technique can be used to either as a measure to avoid unwanted collisions pre-actively or
 * as a way to override a "global" binding so that another value is used within a specific package.
 */
class TestBasicPackageLocalBinds {
  ...
  private static class TestBasicPackageLocalBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      inPackageOf(TestBasicPackageLocalBinds.class).bind(String.class).to("test");
      ...
      in(subPackagesOf(LongBuffer.class, Format.class))
          .bind(String.class)
          .to("java-nio.* & java-text.*");
    }
  }

  private static final Dependency<String> stringGlobal = dependency(String.class);

  private final Injector context = Bootstrap.injector(TestBasicPackageLocalBindsModule.class);
  ...
  void dependencyWithTargetResolvedToSpecificBindInThatPackage() {
    Dependency<String> stringInBind = stringGlobal.injectingInto(TestBasicPackageLocalBinds.class);
  ...
  void dependencyWithTargetResolvedToRelevantSubPackagesBind() {
    Dependency<String> stringInAnnotation =
        stringGlobal.injectingInto(java.lang.annotation.Target.class);
  ...
  void dependencyWithTargetResolvedToRelevantSubPackageOfPackageAndSubPackagesBind() {
    Dependency<String> stringInUtil =
        stringGlobal.injectingInto(java.util.concurrent.Callable.class);
    assertEquals("java-util.*", context.resolve(stringInUtil));
  }
  ...
  void dependencyWithTargetResolvedToRelevantMultiSubPackagesBind() {
    Dependency<String> stringInUtil =
        stringGlobal.injectingInto(java.text.spi.NumberFormatProvider.class);
