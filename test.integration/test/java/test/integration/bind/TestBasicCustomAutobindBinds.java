...
/**
 * This test demonstrates the use of {@link Binder.ScopedBinder#autobind()} in combination with
 * various {@link se.jbee.inject.config.ProducesBy} strategies to semi-automatically bind {@link
 * Constructor}s and/or {@link Method}s as sources of an instances.
 */
class TestBasicCustomAutobindBinds {

  static final StringBuffer STATE = new StringBuffer();

  public static class TestBasicCustomAutobindBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      autobind() //
          .produceBy(OPTIMISTIC.returnTypeAssignableTo(raw(Provider.class))) //
          .in(Implementor2.class);
      autobind() //
          .produceBy(OPTIMISTIC.returnTypeIn(packageAndSubPackagesOf(Injector.class))) //
  ...
  public static class Implementor1 {
    ...
    @WebMethod
    @Resource("Foo")
    public double instanceFactoryMethodWithParameters(@Resource("twenty-one") float factor) {
      return factor * 2d;
    }
  ...
  private final Injector context = Bootstrap.injector(TestBasicCustomAutobindBindsModule.class);
  ...
  /**
   * The provider method {@link Implementor1#instanceFactoryMethodWithParameters(float)} uses the
   * {@link Resource} annotation to specify the name. As a result there is a named {@link Instance}
  ...
  void methodsAreBoundThatAreAssignableToSpecifiedType() {
    assertEquals(true, context.resolve(providerTypeOf(Boolean.class)).provide());
  }
  ...
  void noMethodsAreBoundThatAreNotInSpecifiedPackagesSet() {
    assertThrows(
        UnresolvableDependency.ResourceResolutionFailed.class, () -> context.resolve(String.class));
  }
