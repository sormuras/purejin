...
/**
 * A test that shows how {@link BundleFor}s are used to install one or more (or even none) of a set
 * of options each implying the installation of a different associated {@link
 * se.jbee.inject.bind.Bundle}.
 *
 * <p>This can be used as a classic feature toggle that is configured in the {@link Env} using the
 * {@link Env#withDependent(Class, Enum[])} helper method so set the selected options.
 *
 * <p>Secondly this technique of using {@link Enum}s constant to represent {@link
 * se.jbee.inject.bind.Bundle} classes allows to hide those implementations behind the {@link Enum}
 * that behaves as the API to outside world so it can reference them when installing or uninstalling
 * them.
 *
