...
/**
 * The test shows how to establish loose coupling between software modules using {@link
 * BinderModule#require(Class)} and {@link BinderModule#provide(Class, se.jbee.inject.Hint...)}.
 *
 * <p>One software module expresses the need for a particular implementation of an interface while
 * another software module express the ability to provide an implementation if needed. Interface and
 * implementation are connected without that either of the modules points to the other. Their both
 * are unaware of each other yet the {@link Injector} context connects the two.
 *
 * <p>Such indirect coupling can be useful for example when building a plugin system.
 */
class TestBasicRequiredProvidedBinds {
  ...
  private static class TestBasicRequiredProvidedBindsBundle extends BootstrapperBundle {
  ...
  /**
   * Required and provides can even be used in the presence of an explicit binding for the provided
   * interface. In that case the explicit binding replaces the loose coupling.
   */
  ...
  void requirementIsFulfilledByProvidedBind() {
    Injector injector = Bootstrap.injector(TestBasicRequiredProvidedBindsBundle.class);
    assertNotNull(injector.resolve(ExampleService.class));
  }

  @Test
  void unusedProvidedBindIsNotAddedToInjectorContext() {
    Injector context = Bootstrap.injector(TestBasicRequiredProvidedBindsBundle.class);
    assertThrows(
        UnresolvableDependency.ResourceResolutionFailed.class,
