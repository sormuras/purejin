...
/**
 * Just a short example that shows that the {@link Installs} annotation while being inherited is not
 * effectively inherited when another {@link Installs} annotation is present on a subtype. In such
 * case the annotation is overridden by that instance of the annotation.
 *
 * <p>Hence, {@link Installs} should only be used on leaf classes that are not subject to further
 * inheritance. To install a {@link se.jbee.inject.bind.Bundle} as a "side-effect" of a {@link
 * se.jbee.inject.bind.Module} use the {@link BinderModule#BinderModule(Class)} constructor.
 */
class TestFeatureInstallsAnnotationBinds {
  ...
  void installsOnSupertypesAreOverridden() {
    assertEqualsIgnoreOrder(new String[] {"B", "C"}, context.resolve(String[].class));
  }
}
