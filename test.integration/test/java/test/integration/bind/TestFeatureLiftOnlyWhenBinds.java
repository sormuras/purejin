...
/**
 * Minimal test that shows how {@link Lift#onlyWhen(Predicate)} can be used to add further
 * conditions to a {@link Lift}.
 */
class TestFeatureLiftOnlyWhenBinds {

  private static class TestFeatureLiftOnlyWhenBindsModule extends BinderModule {
  ...
  private final Injector context = Bootstrap.injector(TestFeatureLiftOnlyWhenBindsModule.class);

  @Test
  void liftOnlyWhenFilterIsApplied() {
    assertEquals(3, context.resolve(Integer.class), "Integer did not get lifted as expected");
    assertEquals(
        2L,
