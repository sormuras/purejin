...
/**
 * Primitives and wrapper {@link Class}es are the same {@link Type} as far as the {@link Injector}
 * is concerned.
 */
class TestBasicPrimitiveValueBinds {
  ...
  public static class PrimitiveBindsBean {
    ...
    public PrimitiveBindsBean(int i, float f, boolean b, Integer bigI, Float bigF, Boolean bigB) {
      this.i = i;
      this.f = f;
      this.b = b;
      this.bigI = bigI;
  ...
  private final Injector context = Bootstrap.injector(PrimitiveBindsModule.class);
  ...
  /**
   * To allow such a automatic conversion a bridge can be installed using {@link
   * DefaultFeature#PRIMITIVE_ARRAYS}. By default however this is not installed so the below call
   * fails.
   */
  @Test
  void primitiveArrayNotWorkAsWrapperArrayClasses() {
    assertThrows(ResourceResolutionFailed.class, () -> context.resolve(int[].class));
  }
