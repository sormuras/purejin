...
/**
 * This tests the behaviour of the {@link se.jbee.inject.config.ConstructsBy#OPTIMISTIC} strategy.
 *
 * @see TestBasicHintsBinds for testing of the strategy involving {@link se.jbee.inject.Hint}s.
 */
class TestFeatureOptimisticConstructorBinds {
  ...
  private static class TestFeatureOptimisticConstructorBindsModule extends BinderModule {
  ...
  private final Injector context =
      Bootstrap.injector(TestFeatureOptimisticConstructorBindsModule.class);
  ...
  void constructorsWithParametersOfSameTypeWithGenericsAreNotIgnored() {
    assertNotNull(context.resolve(raw(LegalSelfReference.class).parameterized(Integer.class)));
  }
