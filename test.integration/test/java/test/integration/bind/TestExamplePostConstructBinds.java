...
/**
 * This example shows the effect of the java enterprise {@code PostConstruct} annotation can be
 * added.
 *
 * <p>In this example we use a copy of that annotation as it isn't on the module path. The @{@link
 * PostConstruct} of this example will also allow for the annotated methods to have parameters in
 * which case the {@link Injector} context will try to inject them.
 *
 * <p>This example also shows how a similar concept can be build more elegantly using an interface.
 *
...
class TestExamplePostConstructBinds {
  ...
  private static final class TestExamplePostConstructBindsModule extends BinderModule {
    ...
    private static Object postConstructHook(Object target, Type<?> as, Injector context) {
      // we use the ProducesBy util to find the methods
      Method[] postConstructs =
          ProducesBy.declaredMethods(false) //
              .annotatedWith(PostConstruct.class) //
              .reflect(target.getClass());
      if (postConstructs == null) return target;
      // each found method is called
      for (Method m : postConstructs) call(target, m, context);
      return target;
    }
    ...
    private static <T> void call(Object target, Method m, Injector context) {
      ...
      Supply.byProduction(prod)
          .supply(
              dependency(prod.actualType) //
                  // adds basic targeting so that injectingInto is respected when method arguments
                  // are injected
                  // to get fully correct target context a Supplier<Lift<Object>> would be needed
                  // instead of postConstructHook (Lift<Object>) so that the actual Dependency can
                  // be accessed
  ...
  private final Injector context = Bootstrap.injector(TestExamplePostConstructBindsModule.class);
