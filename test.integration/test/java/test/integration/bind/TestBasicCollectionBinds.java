...
class TestBasicCollectionBinds {
  ...
  private final Injector injector = Bootstrap.injector(AllFeaturesBundle.class);
  ...
  void setIsAvailableForBoundType() {
    ...
    assertInjects(new TreeSet<>(asList(42, 846)), setTypeOf(Integer.class));
  }

  @Test
  void collectionIsAvailable() {
    Type<? extends Collection<?>> collectionType = Cast.collectionTypeOf(Integer.class);
    assertInjectsItems(new Integer[] {846, 42}, collectionType);
  }

  @Test
  void listAsLowerBoundIsAvailable() {
    Type<? extends List<Number>> wildcardListType =
        listTypeOf(Number.class).parameterizedAsUpperBounds();
    assertInjectsItems(new Number[] {846, 42, 42.0f}, wildcardListType);
  }

  @Test
  void setAsLowerBoundIsAvailable() {
    Type<? extends Set<Number>> wildcardSetType =
        setTypeOf(Number.class).parameterizedAsUpperBounds();
    assertInjectsItems(new Number[] {846, 42, 42.0f}, wildcardSetType);
  }

  @Test
  void collectionAsLowerBoundIsAvailable() {
    Type<? extends Collection<Number>> collectionType =
        collectionTypeOf(Number.class).parameterizedAsUpperBounds();
    assertInjectsItems(new Number[] {846, 42, 42.0f}, collectionType);
  }

  @Test
  void listOfListsOfBoundTypesAreAvailable() {
    assertInjects(singletonList(singletonList("foobar")), listTypeOf(listTypeOf(String.class)));
  }

  @Test
  void collectionIsAvailableWhenJustListIsInstalled() {
    Injector injector = Bootstrap.injector(JustListFeatureBundle.class);
    assertInjectsItems(new Integer[] {846, 42}, injector.resolve(collectionTypeOf(Integer.class)));
  }

  @Deprecated
  private <E> void assertInjectsItems(E[] expected, Type<? extends Collection<?>> dependencyType) {
    assertInjectsItems(asList(expected), dependencyType);
  }

  @Deprecated
  private <E> void assertInjectsItems(
      Collection<E> expected, Type<? extends Collection<?>> dependencyType) {
  ...
  private static <E> void assertInjectsItems(Collection<E> expected, Collection<?> actual) {
