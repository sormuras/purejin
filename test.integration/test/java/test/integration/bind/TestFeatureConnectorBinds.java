...
/**
 * This test demonstrates the basic feature of dynamically connection methods for different types of
 * processing.
 */
class TestFeatureConnectorBinds {

  /** Annotations are just one way to select methods used by the test examples in this class */
  @Target(METHOD)
  @Retention(RUNTIME)
  @interface Marked {}

  /**
   * Connecting by default applies to type hierarchies so interfaces can be useful to describe the
   * set of {@link Class}es where member {@link Method}s potentially should be found.
   *
   * <p>In this example the interface is also used to describe the set of {@link Method}s that are
   * considered members.
   */
  @FunctionalInterface
  interface Cleaned {
  ...
  private static class TestFeatureConnectorBindsModule extends BinderModuleWith<Connector> {
    ...
    protected void declare(Connector verifier) {
      ...
      bind("my-cleaned", Connector.class)
          .to(
              (instance, as, method) -> {
  ...
  void interfacesApplyDynamicLinkingToAssignableTypes() {
    ...
    Injector context = Bootstrap.injector(env, TestFeatureConnectorBindsModule.class);
    assertTrue(
        context.resolve("marked", Bean.class).cleaned, "clean was not called for marked bean");
    assertTrue(
        context.resolve("unmarked", Bean.class).cleaned, "clean was not called for unmarked bean");
  }
}
