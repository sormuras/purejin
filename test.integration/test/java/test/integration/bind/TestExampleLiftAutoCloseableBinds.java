...
/**
 * The tests demonstrates how the {@link Lift} and {@link Binder#lift()} can be used to e.g. install
 * a "shutdown hook" that would automatically close all {@link AutoCloseable}s. Here the "shutdown
 * hook" of course is simulated so we can test for it being invoked. In a real scenario one would
 * use {@link Runtime#addShutdownHook(Thread)}.
 */
class TestExampleLiftAutoCloseableBinds {

  static final class TestExampleLiftAutoCloseableBindsModule extends BinderModule
      implements Lift<Injector> {
  ...
  static final class TestExampleLiftAutoCloseableBindsModuleWith extends BinderModuleWith<Integer>
      implements Lift<Injector> {
  ...
  public static class AutoCloseableLift implements Lift<Injector> {
    ...
    public Injector lift(Injector target, Type<?> as, Injector context) {
      ...
      AutoCloseable[] autoCloseable = target.resolve(raw(AutoCloseable[].class).asUpperBound());
  ...
  /** This class simulates some singleton that needs to be closed like a DB connection instance. */
  ...
  void liftCanBeUsedToCloseAnyAutoCloseable() {
    Injector injector = Bootstrap.injector(TestExampleLiftAutoCloseableBindsModule.class);
  ...
  void liftCanMakeUseOfParametersUsingArgumentModules() {
    ...
    TestExampleLiftAutoCloseableBindsModuleWith module =
        (TestExampleLiftAutoCloseableBindsModuleWith) lift;
