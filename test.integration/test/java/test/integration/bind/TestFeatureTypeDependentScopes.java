...
/** A basic test verifying the {@link TypeDependentScope} implementation behaves as expected. */
class TestFeatureTypeDependentScopes {
  ...
  void dependencyTypeScopeEnsuresSingletonPerExactGenericType() {
    ...
    assertSame(
        scope.provide(1, 2, dependency(A.class), null),
        a); // the null Provider shouldn't be called now
    assertSame(scope.provide(2, 2, dependency(B.class), ib), b);
    assertSame(
        scope.provide(2, 2, dependency(B.class), null),
        b); // the null Provider shouldn't be called now
  }
}
