...
/**
 * A test that show how to add an annotation (here {@link ConfigProperty} that will inject the
 * {@link se.jbee.inject.config.Config} value of the same name to the annotated {@link
...
class TestExampleConfigPropertyAnnotationBinds {
  ...
  static UnresolvableDependency.SupplyFailed supplyFailedSupplier() {
    return new UnresolvableDependency.SupplyFailed(
        "No such configuration value or converter", new NoSuchElementException());
  }
  ...
  private static class Solution1 extends BinderModule {
    ...
    private <T> void bindConfigPropertySupplier(Type<T> type) {
      per(Scope.dependencyInstance) //
          ...
          .toSupplier((dep, context) -> resolveConfigProperty(dep, context, dep.at().element()));
    }

    @SuppressWarnings("unchecked")
    private static <T> T resolveConfigProperty(
        Dependency<? super T> dep, Injector context, AnnotatedElement e) {
      ConfigProperty property = e.getAnnotation(ConfigProperty.class);
      Config config =
          context.resolve(dep.uninject().onInstance(Instance.defaultInstanceOf(raw(Config.class))));
      return (T)
          config
              .value(property.from(), property.value())
              .as(dep.type())
  ...
  /**
   * Part of the solution is that we set a {@link HintsBy} strategy that creates the appropriate
   * {@link Hint} that then is resolved to the special {@link Supplier} we have bound in {@link
   * Solution1}.
   */
  private static Injector bootstrapSolution1() {
    return Bootstrap.injector(
        Bootstrap.DEFAULT_ENV.with(
            HintsBy.class,
            HintsBy.instanceReference(
                NamesBy.annotatedWith(ConfigProperty.class, ConfigProperty::value, true))),
        Solution1.class);
  }

  /**
   * An alternative to {@link Solution1} that does not use a {@link Supplier}. Instead the {@link
   * Hint} computed by the custom {@link HintsBy} strategy directly resolves the {@link
   * ConfigProperty} and provides it as a {@link Hint#constant(Object)}.
   *
   * <p>In contrast to {@link Solution1} this solution cannot consider the full {@link Dependency}
   * when resolving the {@link Config}. It is limited to the {@link Class} the {@link
   * java.lang.reflect.Parameter} belongs to. Therefore more sophisticated targeting would not work
   * as expected.
   */
  private static Injector bootstrapSolution2() {
    return Bootstrap.injector(
        Bootstrap.DEFAULT_ENV.with(
            HintsBy.class,
            ((param, context) -> {
              if (!param.isAnnotationPresent(ConfigProperty.class)) return null;
              Config config =
                  context.resolve(
                      dependency(raw(Config.class))
