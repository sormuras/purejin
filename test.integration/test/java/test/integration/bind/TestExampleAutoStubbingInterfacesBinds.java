...
/**
 * This test illustrates how to use binds with {@link Type#asUpperBound()} types (wild-card types)
 * to dynamically yield instances of different types that are all sub-types of a common base class.
 *
 * <p>This can e.g. be used to "automatically" supply stubbing all dependencies that have not be
 * bound explicitly. A single declaration with a custom {@link Supplier} is enough to achieve this.
 */
class TestExampleAutoStubbingInterfacesBinds {
  ...
  private static class TestExampleAutoStubbingInterfacesBindsModule extends BinderModule {
    ...
    private <B> void mockAnyUnbound(Class<? super B> base) {
      ...
      // they provide instances of any type that extends the base (not an instance that would be
      // assignable from base)
      // so a wrongly implemented supplier would lead to a ClassCastExceptions
      @SuppressWarnings("unchecked")
      Type<B> upperBound = (Type<B>) raw(base).asUpperBound();
      per(Scope.injection).bind(anyOf(upperBound)).toSupplier(new MockSupplier<>(base));
    }
  }

  /**
   * This interface is implemented by all mocks to illustrate how this could be used to build "rich"
   * mocks that have a common interface to check/verify mock interaction.
   */
  @FunctionalInterface
  interface Mock {

    /**
     * Here the mocks just count invocations but this could be as sophisticated as needed to verify
     * right behavior.
     */
    int timesInvoked();
  }
  ...
  /**
   * In this example mocks just work for interfaces as {@link Proxy} is used to create the mocks.
   */
  static class MockSupplier<T> implements Supplier<T> {
    ...
    public T supply(Dependency<? super T> dep, Injector context) throws UnresolvableDependency {
      ...
      InvocationHandler handler = new MockInvocationHandler(base, context);
  ...
  static class MockInvocationHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
      ...
      try {
        ...
        dependency =
            dependency
                .injectingInto(Mock.class)
                .injectingInto(method.getDeclaringClass())
                .ignoredScoping();
        ...
        if (rt.isPrimitive()) {
          if (int.class == rt) return 0;
          if (boolean.class == rt) return false;
          // and so forth...
        }
        return null;
      }
  ...
  private final Injector context =
      Bootstrap.injector(TestExampleAutoStubbingInterfacesBindsModule.class);
