...
class TestFeatureBootstrapper {
  ...
  /**
   * One of two bundles in a minimal example of mutual dependent bundles. While this installs {@link
   * OtherMutualDependentBundle} that bundle itself installs this bundle. This should not be a
   * problem and both bundles are just installed once.
   */
  ...
  /**
   * Because the same {@link Locator} is defined twice (the {@link Name#DEFAULT} {@link Integer}
   * instance) this module should cause an exception. All {@link Locator} have to be unique.
   */
  ...
  private static class EagerSingletonsBindsModule extends BinderModule implements Supplier<String> {

    static int eagerCount = 0;

    @Override
    protected void declare() {
      bindLifeCycle(ScopeLifeCycle.singleton.derive(Scope.application).eager());
      bind(named("eager"), String.class).toSupplier(this);
      per(Scope.injection).bind(named("lazy"), String.class).toSupplier(this);
    }
  ...
  private static class CustomConstructorSelectionStrategyModule extends BinderModule {

    @Override
    public Env configure(Env env) {
      return env.with(ConstructsBy.class, OPTIMISTIC.annotatedWith(ConstructFrom.class));
    }
  ...
  void bundlesAreNotBootstrappedMultipleTimesEvenWhenTheyAreMutual() {
    assertTimeout(Duration.ofMillis(100), () -> Bootstrap.injector(OneMutualDependentBundle.class));
  }

  @Test
  void nonUniqueResourcesThrowAnException() {
    assertThrows(
        InconsistentDeclaration.class, () -> Bootstrap.injector(ClashingBindsModule.class));
  }
  ...
  /**
   * In the example {@link Number} is {@link DeclarationType#PUBLISHED} bound for {@link Integer}
   * and {@link Float} but an {@link DeclarationType#EXPLICIT} bind done overrides these automatic
  ...
  void bindingsAreReplacedByMoreQualifiedOnes() {
    ...
    Resource<?>[] rs =
        context.resolve(
            raw(Resource.class)
                .parameterized(Number.class)
                .parameterizedAsUpperBounds()
                .addArrayDimension());
    // TODO can this be limited to cases with a certain Scope so that container can be excluded?
    assertEquals(4, rs.length);
    assertEquals(1, context.resolve(Resource.resourcesTypeOf(Number.class)).length);
    assertEquals(3, context.resolve(Resource.resourcesTypeOf(Comparable.class)).length);
  }
  ...
  void customConstructorSelectionStrategyIsUsedToPickConstructor() {
    Injector injector = Bootstrap.injector(CustomConstructorSelectionStrategyModule.class);
