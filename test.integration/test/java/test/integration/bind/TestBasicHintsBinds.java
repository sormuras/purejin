...
/**
 * The test illustrates how to use {@link Hint}s to give hints which resources should be injected as
 * constructor arguments.
 *
 * <p>Thereby it is important to notice that the list of {@linkplain Hint}s does *not* describe
 * which {@link Constructor} to use! Hence the sequence does *not* has to match the parameter
 * sequence in the constructor definition. As long as types are not assignable a {@linkplain Hint}
 * is tried to used as the next constructor parameter. The first assignable is used.
 *
...
class TestBasicHintsBinds {
  ...
  private static class ParameterConstructorBindsModule extends BinderModule {
    ...
    protected void declare() {
      ...
      bind(Qux.class)
          .toConstructor(
              y.asHint().asType(CharSequence.class), constant(1980).asType(Number.class));
    }
  }

  private static class FaultyParameterConstructorBindsModule extends BinderModule {
  ...
  private final Injector context = Bootstrap.injector(ParameterConstructorBindsModule.class);
  ...
  /**
   * We can see that {@link Hint#asType(Class)} works because the instance y would also been
   * assignable to the 1st parameter of type {@link Serializable} (in {@link Qux}) but since we
   * typed it as a {@link CharSequence} it no longer is assignable to 1st argument and will be used
   * as 2nd argument where the as well {@link Serializable} {@link Number} constant used as 2nd
   * {@link Hint} is used for as the 1st argument.
   */
  @Test
