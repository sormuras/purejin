...
/**
 * This test demonstrates how {@link se.jbee.inject.binder.Binder#detectAt(Class[])} in combination
 * with user {@link java.lang.annotation.Annotation}s can be used to define your own concepts. The
 * effect of such a concept is defined as {@link ModuleWith} (here {@link ServiceAnnotationTemplet})
 * that is added to the {@link Env} which links the user {@link java.lang.annotation.Annotation} and
 * its effect.
 *
 * <p>With such a pattern present in the {@link Env} annotated classes are now simply added using
 * {@link se.jbee.inject.binder.Binder#detectAt(Class[])}. The effects of this follow the pattern as
 * implemented in {@link ServiceAnnotationTemplet} which receives the annotated {@link Class} as an
 * argument.
 *
...
class TestExampleAnnotationGuidedBindingBinds {
  ...
  @interface Contract {
    /**
     * This example shows that the annotations can also have properties which are used when applying
     * the annotation effect.
     */
    Class<?>[] value();
  }
  ...
  /**
   * Applies the effect of the {@link Contract} annotation which binds the class for all named
   * interfaces.
   */
  ...
  /**
   * The separate {@link Bundle} definition should just isolate the {@link ServiceLoader} depended
   * test from the programmatic setup dependent test.
   */
  private static class TestExampleAnnotationGuidedBindingBindsModule extends BinderModule {
  ...
  private final Env env =
      Bootstrap.DEFAULT_ENV //
          ...
          .with(
              named(Provides.class), ModuleWith.METHOD_ANNOTATION, new ProvidesAnnotationTemplet());

  private final Injector context =
      Bootstrap.injector(env, TestExampleAnnotationGuidedBindingBindsModule.class);
  ...
  void customAnnotationsAddedProgrammatically() {
    ...
    assertSame(SomeServiceImpl.class, context.resolve(Serializable.class).getClass());
    // just to check the access limitation is honoured
    assertThrows(IllegalAccess.class, () -> context.resolve(SomeServiceImpl.class));
  }
