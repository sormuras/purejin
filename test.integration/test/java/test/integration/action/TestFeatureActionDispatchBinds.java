...
/**
 * This is a double test. It tests the feature of setting your own {@link ActionDispatch} as well as
 * if the list of {@link se.jbee.inject.action.ActionSite}s provided to the {@link ActionDispatch}
 * is cached by the core as long as no change did occur. That means no suitable {@link
 * java.lang.reflect.Method} connected or disconnected.
 */
class TestFeatureActionDispatchBinds {

  private static class TestFeatureActionDispatchBindsModule extends ActionModule {
  ...
  public static class RecordingActionDispatch<A> implements ActionDispatch<A, A> {
  ...
  private final Injector context = Bootstrap.injector(TestFeatureActionDispatchBindsModule.class);

  @Test
  void listOfActionSitesIsCachedWhenNotChanged() {
    Action<String, String> frontend = context.resolve(actionTypeOf(String.class, String.class));

    Bean backend = context.resolve(Bean.class); // connects the backend
    assertNull(backend.lastParam);

    RecordingActionDispatch<?> strategy = context.resolve(RecordingActionDispatch.class);
