 ...
 * model just describes actual types. There is no representation for a {@link TypeVariable}.
 *
 * <p>Lower bound types ({@code ? super X}) are not supported as they usually are not needed in
...
public final class Type<T>
    implements Qualifying<Type<?>>, Typed<T>, Serializable, Comparable<Type<?>> {
  ...
  public static <T> Type<? extends T> actualInstanceType(T instance, Type<?> as) {
    ...
    if (as.rawType == rawType) return (Type<? extends T>) as;
    TypeVariable<? extends Class<?>>[] generics = rawType.getTypeParameters();
    if (generics.length == 0) return (Type<? extends T>) raw(rawType);
  ...
  public static int typeVariableComparator(TypeVariable<?> a, TypeVariable<?> b) {
    ...
    if (res != 0) return res;
    if (a.getGenericDeclaration().equals(b.getGenericDeclaration())) return 0;
    return Integer.compare(
        a.getGenericDeclaration().hashCode(), b.getGenericDeclaration().hashCode());
  }

  private Type<T> varTypeParametersAsWildcards() {
    TypeVariable<?>[] vars = rawType.getTypeParameters();
    return vars.length == 0 ? this : parameterized(actualTypes(vars, emptyTypeArguments()));
  }
  ...
  public static Type<?> actualReturnType(Constructor<?> member, Type<?> genericDeclaringClass) {
    return genericType(
        member.getAnnotatedReturnType().getType(), actualTypeArguments(genericDeclaringClass));
  }

  public static Type<?> actualParameterType(
      java.lang.reflect.Parameter param, Type<?> genericDeclaringClass) {
  ...
  /**
   * Returns the generic {@link Method} return {@link Type} with any {@link Class} level type
   * parameter replaced with its lower bound, so {@code ?} for an unbound type parameter or {@code
   * Serializable} for {@code ? extends Serializable}.
   *
   * @param method any {@link Method}
   * @return The fully generic return {@link Type} (filling class level type parameters with lower
   *     bounds)
   */
  ...
  public static Type<?>[] actualTypes(
      TypeVariable<?>[] vars, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    Type<?>[] actualTypes = new Type<?>[vars.length];
    for (int i = 0; i < vars.length; i++) actualTypes[i] = actualType(vars[i], actualTypeArguments);
    return actualTypes;
  }

  private static Type<?> actualType(
      TypeVariable<?> var, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    Type<?> actualType = actualTypeArguments.get(var);
    if (actualType != null) return actualType;
    if (actualTypeArguments.containsKey(
        var)) // we have or are in the process of computing that variable
    return Type.WILDCARD;
    actualTypeArguments.put(
        var,
        null); // not yet have it but we want to stop looking it up again so we mark it by putting a
    // null for the name
    actualType =
  ...
  private static Type<?>[] genericTypes(
      java.lang.reflect.Type[] types, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    if (types.length == 0) return new Type[0];
    return Utils.arrayMap(types, Type.class, p -> genericType(p, actualTypeArguments));
  }
  ...
  public static Type<?> genericType(
      java.lang.reflect.Type type, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    if (type instanceof Class<?>) return classType((Class<?>) type);
    if (type instanceof ParameterizedType)
      return genericType((ParameterizedType) type, actualTypeArguments);
    if (type instanceof TypeVariable<?>)
      return actualType((TypeVariable<?>) type, actualTypeArguments);
    if (type instanceof GenericArrayType) {
      GenericArrayType gat = (GenericArrayType) type;
      return genericType(gat.getGenericComponentType(), actualTypeArguments).addArrayDimension();
    }
  ...
  private static <T> Type<T> genericType(
      ParameterizedType type, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    @SuppressWarnings("unchecked")
    Class<T> rawType = (Class<T>) type.getRawType();
    java.lang.reflect.Type[] typeArguments = type.getActualTypeArguments();
    if (typeArguments.length == 1)
      return new Type<>(rawType, new Type[] {genericType(typeArguments[0], actualTypeArguments)});
    return new Type<>(rawType, genericTypes(typeArguments, actualTypeArguments));
  }
  ...
  public <B> Type<?> baseType() {
    if (!rawType.isArray()) return this;
  ...
  public Type<?> parameter(int index) {
    if (index < 0 || index >= rawType.getTypeParameters().length)
      throw new IndexOutOfBoundsException(
          "The type " + this + " has no type parameter at index: " + index);
    return isRawType() ? WILDCARD : params[index];
  }

  public boolean isAssignableTo(Type<?> other) {
    if (!other.rawType.isAssignableFrom(rawType)) return false;
  ...
  public boolean asParameterAssignableTo(Type<?> other) {
    if (rawType == other.rawType) return !isParameterized() || allParametersAreAssignableTo(other);
  ...
  /**
   * @return true if this type describes the upper bound of the required types (a wildcard generic).
   */
  ...
  /**
   * @return true if this {@link Type#isParameterized()} and any of its parameters {@link
   *     #isUpperBound()}
   */
  public boolean isParameterizedAsUpperBound() {
    return isParameterized()
        && Utils.arrayContains(params, p -> p.isUpperBound() || p.isParameterizedAsUpperBound());
  }
  ...
  private static int arrayDimensions(Class<?> type) {
    return !type.isArray() ? 0 : 1 + arrayDimensions(type.getComponentType());
  }

  @Override
  public boolean moreQualifiedThan(Type<?> other) {
    if (!rawType.isAssignableFrom(other.rawType)) return true;
    if ((hasTypeParameter() && !isParameterized()) || (isUpperBound() && !other.isUpperBound()))
      return false; // equal or other is a subtype of this
    if ((other.hasTypeParameter() && !other.isParameterized())
        || (!isUpperBound() && other.isUpperBound())) return true;
    if (rawType == other.rawType) return moreQualifiedParametersThan(other);
    return moreQualifiedParametersThan(other.toSuperType(rawType));
  }

  private boolean moreQualifiedParametersThan(Type<?> other) {
    if (params.length == 1) return params[0].moreQualifiedThan(other.params[0]);
    int moreQualified = 0;
    for (Type<?> param : params) if (param.moreQualifiedThan(other.params[0])) moreQualified++;
  ...
  public Type<T> parameterizedAsUpperBounds() {
    if (!isParameterized())
      return isRawType()
          ? parameterized(actualTypes(rawType.getTypeParameters(), emptyTypeArguments()))
          : this;
    if (allTypeParametersAreUpperBounds()) return this;
    return new Type<>(upperBound, rawType, Utils.arrayMap(params, Type::asUpperBound));
  }
  ...
  private void toString(StringBuilder str, boolean fullName) {
    ...
    String name = fullName ? rawType.getCanonicalName() : rawType.getSimpleName();
    if (name == null) // fallback for anonymous inner classes which have no canonical or simple name
    name = rawType.getName();
    str.append(rawType.isArray() ? name.substring(0, name.indexOf('[')) : name);
    if (isParameterized()) {
      str.append('<');
    ...
    if (rawType.isArray()) str.append(name.substring(name.indexOf('[')));
  }
  ...
  private void checkTypeParameters(Type<?>... args) {
    if (args.length == 0) return; // is treated as raw-type
  ...
  private void checkTypeParameters(
      TypeVariable<Class<T>> var, Type<?> arg, Map<TypeVariable<?>, Type<?>> actualTypeArguments) {
    for (java.lang.reflect.Type genericUpperBound : var.getBounds()) {
      ...
      if (varType != null && arg.equalTo(varType)) return; // recursive definition
      if (genericUpperBound != Object.class && !arg.isAssignableTo(upperBoundType)) {
        throw new IllegalArgumentException(
            arg + " is not assignable to the type variable: " + upperBoundType);
      }
    }
  }

  /**
   * Returns the actual super-type of this {@link Type} (including its actual type parameters) for
   * the given super-{@link Class}.
   *
   * @param rawSuperType a {@link Class} reference to a super-class or super-interface of this
   *     {@link Type}. In other words a {@link Class} type this {@link Type} would be assignable to.
   * @return the actual super-{@link Type}
   * @throws ClassCastException if the given {@link Class} is no super-type of this {@link Type}
   * @see #castTo(Type)
   */
  @SuppressWarnings("unchecked")
  public Type<? super T> toSuperType(Class<?> rawSuperType) {
     ...
     */
    if (rawSuperType == rawType) return this;
    if (rawSuperType.getTypeParameters().length == 0) {
      if (rawSuperType.isAssignableFrom(rawType)) return (Type<? super T>) raw(rawSuperType);
      failedCastTo(rawSuperType);
    }
    Object[] box = new Object[1];
    walkSuperTypes(
        this,
        !rawSuperType.isInterface(),
        rawSuperType.isInterface(),
        true,
        t -> {
    ...
    if (box[0] == null) failedCastTo(rawSuperType);
  ...
  /**
   * @return a list of all super-classes and super-interfaces of this type starting with the direct
   *     super-class followed by the direct super-interfaces continuing by going up the type
   *     hierarchy.
   */
  ...
  /**
   * Walks the inheritance tree and passes all actual types implemented by given {@code type} to the
   * consumer.
   *
   * @return true, if the walking was cancelled by the consumer returning false and while
   *     cancelOnFalse is true.
   */
  private static boolean walkSuperTypes(
      Type<?> type,
      boolean walkClassTypes,
      boolean walkInterfaceTypes,
      boolean cancelOnFalse,
      Predicate<Type<?>> consumer) {
    Class<?> supertype = type.rawType.getSuperclass();
    Map<TypeVariable<?>, Type<?>> actualTypeArguments = actualTypeArguments(type);
    if (walkInterfaceTypes
        && !walkSuperInterfaces(type.rawType, actualTypeArguments, cancelOnFalse, consumer))
      return false;
    if (supertype == null) return true; // done
    Type<?> superclass = genericType(type.rawType.getGenericSuperclass(), actualTypeArguments);
    if (walkClassTypes && !consumer.test(superclass) && cancelOnFalse) return false;
    return walkSuperTypes(superclass, walkClassTypes, walkInterfaceTypes, cancelOnFalse, consumer);
  }
   ...
   *     cancelOnFalse is true.
   */
  private static boolean walkSuperInterfaces(
      Class<?> type,
    ...
    for (int i = 0; i < ix.length; i++) {
      Type<?> interfaceType = Type.genericType(gix[i], actualTypeArguments);
      boolean res =
          consumer.test(interfaceType)
              && walkSuperInterfaces(
                  ix[i], actualTypeArguments(interfaceType), cancelOnFalse, consumer);
      if (!res && cancelOnFalse) return false;
    }
    return true;
  }
  ...
  private static <V> Map<TypeVariable<?>, Type<?>> actualTypeArguments(Type<V> type) {
    ...
    for (int i = 0; i < vars.length; i++) {
      // it would be correct to use the joint type of the bounds but since it is not possible to
      // create a type with illegal parameters it is ok to just use Object since there is no way to
      // model a joint type
