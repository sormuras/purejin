...
/**
 * When determining what resource is used to inject a dependency everything is sorted starting with
 * the most {@link Qualifying}. The most qualified match is injected.
 *
...
public interface Qualifying<T extends Qualifying<T>> {
   ...
   *     <p>Equal objects are do not define one of them as more qualified! Also objects that have no
   *     common context or relationship don't define one of them as more qualified.
   *     <p>For example two {@link Type}s with no common super-type do not define one of them as
   *     more qualified.
   */
  ...
  static <A extends Qualifying<? super A>> int compare(A one, A other) {
    return one.moreQualifiedThan(other) ? -1 : other.moreQualifiedThan(one) ? 1 : 0;
  }
}
